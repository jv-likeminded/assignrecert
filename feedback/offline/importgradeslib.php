 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignrecertfeedback_offline_grade_importer {
    ...
    /** @var assignmentrecert $assignmentrecert - the assignmentrecert class */
    private $assignmentrecert;
     ...
     * @param assignrecert $assignmentrecert The current assignmentrecert
     */
    public function __construct($importid, assignrecert $assignmentrecert, $encoding = 'utf-8', $separator = 'comma') {
    ...
    public function parsecsv($csvdata) {
        $this->csvreader = new csv_import_reader($this->importid, 'assignrecertfeedback_offline');
    ...
    public function init() {
        ...
        $strid = get_string('recordid', 'assignrecert');
        $strmodified = get_string('lastmodifiedgrade', 'assignrecert');
        ...
        $this->validusers = $this->assignmentrecert->list_participants($groupid, false);
        return true;
    }
    ...
    public function next() {
        ...
        while ($record = $this->csvreader->next()) {
            ...
            $id = substr($idstr, strlen(get_string('hiddenuser', 'assignrecert')));
            if ($userid = $this->assignmentrecert->get_user_id_for_uniqueid($id)) {
