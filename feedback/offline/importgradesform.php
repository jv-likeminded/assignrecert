 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignrecertfeedback_offline_import_grades_form extends moodleform implements renderable {
    ...
    public function definition() {
        ...
        $renderer = $PAGE->get_renderer('assignrecert');

        // Visible elements.
        $assignmentrecert = $params['assignmentrecert'];
        ...
        $scaleoptions = null;
        if ($assignmentrecert->get_instance()->grade < 0) {
            if ($scale = $DB->get_record('scale', array('id'=>-($assignmentrecert->get_instance()->grade)))) {
        ...
        if (!$gradeimporter->init()) {
            $thisurl = new moodle_url('/mod/assignrecert/view.php', array('action'=>'viewpluginpage',
                                                                     ...
                                                                     'id'=>$assignmentrecert->get_course_module()->id));
            print_error('invalidgradeimport', 'assignrecertfeedback_offline', $thisurl);
            return;
        }

        $mform->addElement('header', 'importgrades', get_string('importgrades', 'assignrecertfeedback_offline'));
        ...
        while ($record = $gradeimporter->next()) {
            ...
            if ($assignmentrecert->is_blind_marking()) {
                $userdesc = get_string('hiddenuser', 'assignrecert') . $assignmentrecert->get_uniqueid_for_user($user->id);
            }

            $usergrade = $assignmentrecert->get_user_grade($user->id, false);
            ...
            if ($usergrade && $usergrade->grade == $grade) {
                ...
                $skip = true;
            } else if ($assignmentrecert->grading_disabled($user->id)) {
                // Skip grade is locked.
                $skip = true;
            } else if (($assignmentrecert->get_instance()->grade > -1) &&
                      (($grade < 0) || ($grade > $assignmentrecert->get_instance()->grade))) {
                // Out of range.
                $skip = true;
            }
            ...
            if (!$skip) {
                ...
                if (!empty($scaleoptions)) {
                    ...
                    $gradeitem = $assignmentrecert->get_grade_item();
                    $formattedgrade = format_float($grade, $gradeitem->get_decimals());
                }
                $updates[] = get_string('gradeupdate', 'assignrecertfeedback_offline',
            ...
            if ($ignoremodified || !$stalemodificationdate) {
                foreach ($record->feedback as $feedback) {
                    ...
                    if ($newvalue != $oldvalue) {
                        $update = true;
                        $updates[] = get_string('feedbackupdate', 'assignrecertfeedback_offline',
        ...
        if ($update) {
            ...
            $mform->addElement('html', get_string('nochanges', 'assignrecertfeedback_offline'));
        }

        $mform->addElement('hidden', 'id', $assignmentrecert->get_course_module()->id);
        ...
        $mform->addElement('hidden', 'pluginsubtype', 'assignrecertfeedback');
