...
/**
 * Unit tests for assignrecertfeedback_editpdf\comments_quick_list
 *
...
use \assignrecertfeedback_editpdf\pdf;
use \assignrecertfeedback_editpdf\comment;
use \assignrecertfeedback_editpdf\annotation;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignrecertfeedback_editpdf_testcase extends advanced_testcase {
    ...
    protected function require_ghostscript() {
        ...
        if ($result->status !== assignrecertfeedback_editpdf\pdf::GSPATH_OK) {
            $this->markTestSkipped('Ghostscript not setup');
        }
    }

    protected function add_file_submission($student, $assignrecert) {
        global $CFG;
        ...
        $pdfsubmission = (object) array(
            'contextid' => $assignrecert->get_context()->id,
        ...
        $sourcefile = $CFG->dirroot.'/mod/assignrecert/feedback/editpdf/tests/fixtures/submission.pdf';
    ...
    public function test_page_editor() {
        ...
        $assignrecert = $this->create_instance($course, [
                ...
                'assignrecertfeedback_editpdf_enabled' => 1,
                'assignrecertsubmission_file_maxsizebytes' => 1000000,
            ]);

        // Add the standard submission.
        $this->add_file_submission($student, $assignrecert);

        $this->setUser($teacher);

        $grade = $assignrecert->get_user_grade($student->id, true);
    ...
    public function test_document_services() {
        ...
        document_services::get_page_images_for_attempt($assignrecert, $student->id, -1);
        $combinedpdf = $fs->get_file($contextid, $component, $filearea, $itemid, $filepath, $filename);
        ...
        $file = document_services::generate_feedback_document($assignrecert->get_instance()->id, $grade->userid, $grade->attemptnumber);
        $this->assertNotEmpty($file);

        $file2 = document_services::get_feedback_document($assignrecert->get_instance()->id, $grade->userid, $grade->attemptnumber);

        $this->assertEquals($file, $file2);

        document_services::delete_feedback_document($assignrecert->get_instance()->id, $grade->userid, $grade->attemptnumber);
        $file3 = document_services::get_feedback_document($assignrecert->get_instance()->id, $grade->userid, $grade->attemptnumber);
    ...
    public function test_is_feedback_modified() {
        ...
        $plugin = $assignrecert->get_feedback_plugin_by_type('editpdf');
        $data = new stdClass();
