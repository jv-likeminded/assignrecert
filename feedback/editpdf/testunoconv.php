...
$PAGE->set_url(new moodle_url('/mod/assignrecert/feedback/editpdf/testunoconv.php'));
$PAGE->set_context(context_system::instance());

require_login();
require_capability('moodle/site:config', context_system::instance());

$strheading = get_string('test_unoconv', 'assignrecertfeedback_editpdf');
...
$PAGE->navbar->add(get_string('pluginname', 'assignrecertfeedback_editpdf'),
        new moodle_url('/admin/settings.php', array('section' => 'assignrecertfeedback_editpdf')));
...
switch ($result->status) {
    case file_storage::UNOCONVPATH_OK:
        $msg = $OUTPUT->notification(get_string('test_unoconvok', 'assignrecertfeedback_editpdf'), 'success');
        $pdflink = new moodle_url($PAGE->url, array('sendpdf' => 1, 'sesskey' => sesskey()));
        $msg .= html_writer::link($pdflink, get_string('test_unoconvdownload', 'assignrecertfeedback_editpdf'));
    ...
    default:
        $msg = $OUTPUT->notification(get_string("test_unoconv{$result->status}", 'assignrecertfeedback_editpdf'), 'warning');
        break;
}
