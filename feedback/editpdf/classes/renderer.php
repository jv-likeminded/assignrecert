 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignrecertfeedback_editpdf_renderer extends plugin_renderer_base {
    ...
    private function render_toolbar_button($icon, $tool, $accesskey = null, $disabled=false) {
        ...
        $alttext->tool = get_string($tool, 'assignrecertfeedback_editpdf');
        if (!empty($accesskey)) {
            $alttext->shortcut = '(Alt/Shift-Alt/Ctrl-Option + ' . $accesskey . ')';
        } else {
            $alttext->shortcut = '';
        }
        $iconalt = get_string('toolbarbutton', 'assignrecertfeedback_editpdf', $alttext);

        $iconhtml = $this->pix_icon($icon, $iconalt, 'assignrecertfeedback_editpdf');
     ...
     *
     * @param assignrecertfeedback_editpdf_widget $widget - Renderable widget containing assignmentrecert, user and attempt number.
     * @return string
     */
    public function render_assignrecertfeedback_editpdf_widget(assignrecertfeedback_editpdf_widget $widget) {
        global $CFG;

        $html = '';

        $html .= html_writer::div(get_string('jsrequired', 'assignrecertfeedback_editpdf'), 'hiddenifjs');
        $linkid = html_writer::random_id();
        if ($widget->readonly) {
            $launcheditorlink = html_writer::tag('a',
                                              get_string('viewfeedbackonline', 'assignrecertfeedback_editpdf'),
                                              array('id'=>$linkid, 'class'=>'btn btn-default', 'href'=>'#'));
        } else {
            $launcheditorlink = html_writer::tag('a',
                                              get_string('launcheditor', 'assignrecertfeedback_editpdf'),
                                              array('id'=>$linkid, 'class'=>'btn btn-default', 'href'=>'#'));
        }
        $links = $launcheditorlink;
        $html .= '<input type="hidden" name="assignrecertfeedback_editpdf_haschanges" value="false"/>';

        $html .= html_writer::div($links, 'visibleifjs');
        $header = get_string('pluginname', 'assignrecertfeedback_editpdf');
        $body = '';
        ...
        $iconalt = get_string('navigateprevious', 'assignrecertfeedback_editpdf');
        $iconhtml = $this->pix_icon($nav_prev, $iconalt, 'assignrecertfeedback_editpdf');
        ...
        $navigation1 .= html_writer::tag('select', null, array('disabled'=>'true',
            'aria-label' => get_string('gotopage', 'assignrecertfeedback_editpdf'), 'class'=>'navigate-page-select',
            'accesskey' => $this->get_shortcut('navigate-page-select')));
        $iconalt = get_string('navigatenext', 'assignrecertfeedback_editpdf');
        $iconhtml = $this->pix_icon($nav_next, $iconalt, 'assignrecertfeedback_editpdf');
        $navigation1 .= html_writer::tag('button', $iconhtml, array('disabled'=>'true',
        ...
        $progressbar = html_writer::div($progressbar, 'progress progress-info progress-striped active',
            array('title' => get_string('loadingeditor', 'assignrecertfeedback_editpdf'),
                  'role'=> 'progressbar', 'aria-valuenow' => 0, 'aria-valuemin' => 0,
                  'aria-valuemax' => 100));
        $progressbarlabel = html_writer::div(get_string('generatingpdf', 'assignrecertfeedback_editpdf'),
            'progressbarlabel');
        ...
        $changesmessage = html_writer::tag('div',
                                           get_string('draftchangessaved', 'assignrecertfeedback_editpdf'),
                                           array(
                                               'class' => 'assignrecertfeedback_editpdf_unsavedchanges warning label label-info'
                                           ));
        ...
        $editorparams = array(array('header'=>$header,
                                    ...
                                    'footer'=>$footer,
                                    'linkid'=>$linkid,
                                    'assignmentrecertid'=>$widget->assignmentrecert,
        ...
        $this->page->requires->yui_module('moodle-assignrecertfeedback_editpdf-editor',
