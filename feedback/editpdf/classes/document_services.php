...
/**
 * This file contains the ingest manager for the assignrecertfeedback_editpdf plugin
 *
...
namespace assignrecertfeedback_editpdf;
...
EOD;
    ...
    /**
     * This function will take an int or an assignmentrecert instance and
     * return an assignmentrecert instance. It is just for convenience.
     * @param int|\assignrecert $assignmentrecert
     * @return assignrecert
     */
    private static function get_assignmentrecert_from_param($assignmentrecert) {
        global $CFG;

        require_once($CFG->dirroot . '/mod/assignrecert/locallib.php');

        if (!is_object($assignmentrecert)) {
            $cm = \get_coursemodule_from_instance('assignrecert', $assignmentrecert, 0, false, MUST_EXIST);
     ...
     */
    private static function hash($assignmentrecert, $userid, $attemptnumber) {
        if (is_object($assignmentrecert)) {
            $assignmentrecertid = $assignmentrecert->get_instance()->id;
        } else {
            $assignmentrecertid = $assignmentrecert;
        }
        return sha1($assignmentrecertid . '_' . $userid . '_' . $attemptnumber);
    }
     ...
     */
    public static function list_compatible_submission_files_for_attempt($assignmentrecert, $userid, $attemptnumber) {
        global $USER, $DB;

        $assignmentrecert = self::get_assignmentrecert_from_param($assignmentrecert);
        ...
        if ($assignmentrecert->get_instance()->teamsubmission) {
            $submission = $assignmentrecert->get_group_submission($userid, 0, false, $attemptnumber);
        } else {
            $submission = $assignmentrecert->get_user_submission($userid, false, $attemptnumber);
        }
            ...
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                ...
                foreach ($pluginfiles as $filename => $file) {
                    if ($file instanceof \stored_file) {
                        ...
                        $record = new \stdClass();
                        $record->contextid = $assignmentrecert->get_context()->id;
                        $record->component = 'assignrecertfeedback_editpdf';
     ...
     * @return stored_file
     */
    public static function get_combined_pdf_for_attempt($assignmentrecert, $userid, $attemptnumber) {
        ...
        if (!$combinedpdf = $fs->get_file($contextid, $component, $filearea, $itemid, $filepath, $filename)) {
            return self::generate_combined_pdf_for_attempt($assignmentrecert, $userid, $attemptnumber);
        }
     ...
     * @return stored_file
     */
    public static function generate_combined_pdf_for_attempt($assignmentrecert, $userid, $attemptnumber) {
        global $CFG;
        ...
        $files = self::list_compatible_submission_files_for_attempt($assignmentrecert, $userid, $attemptnumber);
        ...
        if ($files) {
            ...
            $tmpdir = \make_temp_directory('assignrecertfeedback_editpdf/combined/' . self::hash($assignmentrecert, $userid, $attemptnumber));
     ...
     */
    public static function page_number_for_attempt($assignmentrecert, $userid, $attemptnumber, $readonly = false) {
        global $CFG;
        ...
        if ($readonly) {
            $grade = $assignmentrecert->get_user_grade($userid, true, $attemptnumber);
            $fs = get_file_storage();
            $files = $fs->get_directory_files($assignmentrecert->get_context()->id, 'assignrecertfeedback_editpdf',
        ...
        $file = self::get_combined_pdf_for_attempt($assignmentrecert, $userid, $attemptnumber);
        if (!$file) {
            \print_error('Could not generate combined pdf.');
        }

        // Store the combined pdf file somewhere to be opened by tcpdf.
        $tmpdir = \make_temp_directory('assignrecertfeedback_editpdf/pagetotal/'
            . self::hash($assignmentrecert, $userid, $attemptnumber));
     ...
     * @return array(stored_file)
     */
    public static function generate_page_images_for_attempt($assignmentrecert, $userid, $attemptnumber) {
        global $CFG;
        ...
        $tmpdir = \make_temp_directory('assignrecertfeedback_editpdf/pageimages/' . self::hash($assignmentrecert, $userid, $attemptnumber));
        $combined = $tmpdir . '/' . self::COMBINED_PDF_FILENAME;
        $file->copy_content_to($combined); // Copy the file.
     ...
     * @return array(stored_file)
     */
    public static function get_page_images_for_attempt($assignmentrecert, $userid, $attemptnumber, $readonly = false) {
        global $DB;
        ...
        if ($readonly) {
            ...
            if ($fs->is_area_empty($contextid, $component, $filearea, $itemid)) {
                ...
                self::generate_page_images_for_attempt($assignmentrecert, $userid, $attemptnumber);
                self::copy_pages_to_readonly_area($assignmentrecert, $grade);
            }
        }
        ...
        if (empty($pages)) {
            ...
            $pages = self::generate_page_images_for_attempt($assignmentrecert, $userid, $attemptnumber);
        }
     ...
     * @return string
     */
    protected static function get_downloadable_feedback_filename($assignmentrecert, $userid, $attemptnumber) {
        global $DB;

        $assignmentrecert = self::get_assignmentrecert_from_param($assignmentrecert);

        $groupmode = groups_get_activity_groupmode($assignmentrecert->get_course_module());
        $groupname = '';
        if ($groupmode) {
            $groupid = groups_get_activity_group($assignmentrecert->get_course_module(), true);
        ...
        if ($assignmentrecert->is_blind_marking()) {
            $prefix = $groupname . get_string('participant', 'assignrecert');
            $prefix = str_replace('_', ' ', $prefix);
            $prefix = clean_filename($prefix . '_' . $assignmentrecert->get_uniqueid_for_user($userid) . '_');
        } else {
     ...
     * @return stored_file
     */
    public static function generate_feedback_document($assignmentrecert, $userid, $attemptnumber) {
        ...
        $tmpdir = \make_temp_directory('assignrecertfeedback_editpdf/final/' . self::hash($assignmentrecert, $userid, $attemptnumber));
        $combined = $tmpdir . '/' . self::COMBINED_PDF_FILENAME;
        $file->copy_content_to($combined); // Copy the file.
        ...
        $fs = \get_file_storage();
        $stamptmpdir = \make_temp_directory('assignrecertfeedback_editpdf/stamps/' . self::hash($assignmentrecert, $userid, $attemptnumber));
        $grade = $assignmentrecert->get_user_grade($userid, true, $attemptnumber);
        // Copy any new stamps to this instance.
        if ($files = $fs->get_area_files($assignmentrecert->get_context()->id,
        ...
        $filename = self::get_downloadable_feedback_filename($assignmentrecert, $userid, $attemptnumber);
     ...
     *
     * @param int|\assignrecert $assignmentrecert The assignmentrecert.
     * @param \stdClass $grade The grade record.
     * @return void
     */
    public static function copy_pages_to_readonly_area($assignmentrecert, $grade) {
        $fs = get_file_storage();
        $assignmentrecert = self::get_assignmentrecert_from_param($assignmentrecert);
        $contextid = $assignmentrecert->get_context()->id;
        $component = 'assignrecertfeedback_editpdf';
     ...
     * @return stored_file
     */
    public static function get_feedback_document($assignmentrecert, $userid, $attemptnumber) {
     ...
     * @return bool
     */
    public static function delete_feedback_document($assignmentrecert, $userid, $attemptnumber) {
