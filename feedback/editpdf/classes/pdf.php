...
class pdf extends \FPDI {
    ...
    const BLANK_PDF = '/mod/assignrecert/feedback/editpdf/fixtures/blank.pdf';
    ...
    public function get_image($pageno) {
        ...
        if ($generate) {
            ...
            if (!file_exists($imagefile)) {
                $fullerror = '<pre>' . get_string('output', 'assignrecertfeedback_editpdf')."\n";
                $fullerror .= htmlspecialchars(implode("\n",$output)) . '</pre>';
                throw new \moodle_exception('errorgenerateimage', 'assignrecertfeedback_editpdf', '', $fullerror);
            }
        }
    ...
    public static function ensure_pdf_compatible(\stored_file $file) {
        ...
        $temparea = \make_temp_directory('assignrecertfeedback_editpdf');
    ...
    public static function get_error_image($errorimagefolder, $pageno) {
        ...
        $pdf->copy_page();
        $pdf->add_comment(get_string('errorpdfpage', 'assignrecertfeedback_editpdf'), 250, 300, 200, "red");
    ...
    public static function test_gs_path($generateimage = true) {
        ...
        $testfile = $CFG->dirroot.'/mod/assignrecert/feedback/editpdf/tests/fixtures/testgs.pdf';
        if (!file_exists($testfile)) {
            $ret->status = self::GSPATH_NOTESTFILE;
            return $ret;
        }

        $testimagefolder = \make_temp_directory('assignrecertfeedback_editpdf_test');
