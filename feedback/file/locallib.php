...
// File areas for file feedback assignmentrecert.
define('ASSIGNRECERTFEEDBACK_FILE_FILEAREA', 'feedback_files');
define('ASSIGNRECERTFEEDBACK_FILE_BATCH_FILEAREA', 'feedback_files_batch');
define('ASSIGNRECERTFEEDBACK_FILE_IMPORT_FILEAREA', 'feedback_files_import');
define('ASSIGNRECERTFEEDBACK_FILE_MAXSUMMARYFILES', 5);
define('ASSIGNRECERTFEEDBACK_FILE_MAXSUMMARYUSERS', 5);
define('ASSIGNRECERTFEEDBACK_FILE_MAXFILEUNZIPTIME', 120);
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignrecert_feedback_file extends assignrecert_feedback_plugin {
    ...
    public function get_name() {
        return get_string('file', 'assignrecertfeedback_file');
    }
    ...
    public function get_file_feedback($gradeid) {
        global $DB;
        return $DB->get_record('assignrecertfeedback_file', array('grade'=>$gradeid));
    }
    ...
    public function is_feedback_modified(stdClass $grade, stdClass $data) {
        ...
        if (isset($filekey)) {
            ...
            $filecount = $this->count_files($grade->id, ASSIGNRECERTFEEDBACK_FILE_FILEAREA);
    ...
    public function update_file_count($grade) {
        ...
        if ($filefeedback) {
            $filefeedback->numfiles = $this->count_files($grade->id, ASSIGNRECERTFEEDBACK_FILE_FILEAREA);
            return $DB->update_record('assignrecertfeedback_file', $filefeedback);
        } else {
            $filefeedback = new stdClass();
            $filefeedback->numfiles = $this->count_files($grade->id, ASSIGNRECERTFEEDBACK_FILE_FILEAREA);
            $filefeedback->grade = $grade->id;
            $filefeedback->assignmentrecert = $this->assignmentrecert->get_instance()->id;
            return $DB->insert_record('assignrecertfeedback_file', $filefeedback) > 0;
        }
    }
    ...
    public function view_summary(stdClass $grade, & $showviewlink) {
        ...
        $count = $this->count_files($grade->id, ASSIGNRECERTFEEDBACK_FILE_FILEAREA);

        // Show a view all link if the number of files is over this limit.
        $showviewlink = $count > ASSIGNRECERTFEEDBACK_FILE_MAXSUMMARYFILES;

        if ($count <= ASSIGNRECERTFEEDBACK_FILE_MAXSUMMARYFILES) {
            return $this->assignmentrecert->render_area_files('assignrecertfeedback_file',
                                                        ASSIGNRECERTFEEDBACK_FILE_FILEAREA,
                                                        $grade->id);
        } else {
            return get_string('countfiles', 'assignrecertfeedback_file', $count);
        }
    }
    ...
    public function delete_instance() {
        ...
        $DB->delete_records('assignrecertfeedback_file',
    ...
    public function is_empty(stdClass $grade) {
        return $this->count_files($grade->id, ASSIGNRECERTFEEDBACK_FILE_FILEAREA) == 0;
    }
    ...
    public function get_file_areas() {
        return array(ASSIGNRECERTFEEDBACK_FILE_FILEAREA=>$this->get_name());
    }
    ...
    public function get_grading_batch_operations() {
        return array('uploadfiles'=>get_string('uploadfiles', 'assignrecertfeedback_file'));
    }
    ...
    public function view_batch_upload_files($users) {
        ...
        require_once($CFG->dirroot . '/mod/assignrecert/feedback/file/batchuploadfilesform.php');
        require_once($CFG->dirroot . '/mod/assignrecert/renderable.php');

        $formparams = array('cm'=>$this->assignmentrecert->get_course_module()->id,
                            'users'=>$users,
        ...
        foreach ($users as $userid) {
            if ($usercount >= ASSIGNRECERTFEEDBACK_FILE_MAXSUMMARYUSERS) {
                $moreuserscount = count($users) - ASSIGNRECERTFEEDBACK_FILE_MAXSUMMARYUSERS;
                $usershtml .= get_string('moreusers', 'assignrecertfeedback_file', $moreuserscount);
                break;
            }
            $user = $DB->get_record('user', array('id'=>$userid), '*', MUST_EXIST);

            $usersummary = new assignrecert_user_summary($user,
                                                   $this->assignmentrecert->get_course()->id,
                                                   has_capability('moodle/site:viewfullnames',
                                                   $this->assignmentrecert->get_course_context()),
                                                   $this->assignmentrecert->is_blind_marking(),
                                                   $this->assignmentrecert->get_uniqueid_for_user($user->id),
                                                   get_extra_user_fields($this->assignmentrecert->get_context()));
            $usershtml .= $this->assignmentrecert->get_renderer()->render($usersummary);
            $usercount += 1;
        }

        $formparams['usershtml'] = $usershtml;

        $mform = new assignrecertfeedback_file_batch_upload_files_form(null, $formparams);
        ...
        if ($mform->is_cancelled()) {
            ...
            $data = file_postupdate_standard_filemanager($data,
                                                         ...
                                                         $this->assignmentrecert->get_context(),
                                                         'assignrecertfeedback_file',
                                                         ASSIGNRECERTFEEDBACK_FILE_BATCH_FILEAREA,
                                                         $USER->id);
            $fs = get_file_storage();

            // Now copy each of these files to the users feedback file area.
            foreach ($users as $userid) {
                $grade = $this->assignmentrecert->get_user_grade($userid, true);
                ...
                if ($filefeedback) {
                    $filefeedback->numfiles = $this->count_files($grade->id,
                                                                 ASSIGNRECERTFEEDBACK_FILE_FILEAREA);
                    $DB->update_record('assignrecertfeedback_file', $filefeedback);
                } else {
                    $filefeedback = new stdClass();
                    $filefeedback->numfiles = $this->count_files($grade->id,
                                                                 ASSIGNRECERTFEEDBACK_FILE_FILEAREA);
                    $filefeedback->grade = $grade->id;
                    $filefeedback->assignmentrecert = $this->assignmentrecert->get_instance()->id;
                    $DB->insert_record('assignrecertfeedback_file', $filefeedback);
                }
            }

            // Now delete the temporary import area.
            $fs->delete_area_files($this->assignmentrecert->get_context()->id,
                                   'assignrecertfeedback_file',
                                   ASSIGNRECERTFEEDBACK_FILE_BATCH_FILEAREA,
                                   $USER->id);
            ...
            $header = new assignrecert_header($this->assignmentrecert->get_instance(),
                                        $this->assignmentrecert->get_context(),
                                        false,
                                        $this->assignmentrecert->get_course_module()->id,
                                        get_string('batchuploadfiles', 'assignrecertfeedback_file'));
            $o = '';
            $o .= $this->assignmentrecert->get_renderer()->render($header);
            $o .= $this->assignmentrecert->get_renderer()->render(new assignrecert_form('batchuploadfiles', $mform));
            $o .= $this->assignmentrecert->get_renderer()->render_footer();
        }
    ...
    public function view_upload_zip() {
        ...
        require_capability('mod/assignrecert:grade', $this->assignmentrecert->get_context());
        require_once($CFG->dirroot . '/mod/assignrecert/feedback/file/uploadzipform.php');
        require_once($CFG->dirroot . '/mod/assignrecert/feedback/file/importziplib.php');
        require_once($CFG->dirroot . '/mod/assignrecert/feedback/file/importzipform.php');

        $formparams = array('context'=>$this->assignmentrecert->get_context(),
                            'cm'=>$this->assignmentrecert->get_course_module()->id);
        $mform = new assignrecertfeedback_file_upload_zip_form(null, $formparams);
        ...
        $importer = new assignrecertfeedback_file_zip_importer();
        ...
        if ($mform->is_cancelled()) {
            ...
            $params = array('assignmentrecert'=>$this->assignmentrecert, 'importer'=>$importer);

            $mform = new assignrecertfeedback_file_import_zip_form(null, $params);
            if ($mform->is_cancelled()) {
                $importer->delete_import_files($contextid);
                $urlparams = array('id'=>$this->assignmentrecert->get_course_module()->id,
                                   'action'=>'grading');
            ...
            $o .= $importer->import_zip_files($this->assignmentrecert, $this);
            $importer->delete_import_files($contextid);
        } else if (($data = $mform->get_data()) &&
            ...
            $header = new assignrecert_header($this->assignmentrecert->get_instance(),
                                        ...
                                        false,
                                        $this->assignmentrecert->get_course_module()->id,
                                        get_string('confirmuploadzip', 'assignrecertfeedback_file'));
            $o .= $renderer->render($header);
            $o .= $renderer->render(new assignrecert_form('confirmimportzip', $mform));
            ...
            $header = new assignrecert_header($this->assignmentrecert->get_instance(),
                                        ...
                                        false,
                                        $this->assignmentrecert->get_course_module()->id,
                                        get_string('uploadzip', 'assignrecertfeedback_file'));
            $o .= $renderer->render($header);
            $o .= $renderer->render(new assignrecert_form('uploadfeedbackzip', $mform));
            $o .= $renderer->render_footer();
        }
    ...
    /**
     * Called by the assignmentrecert module when someone chooses something from the
    ...
    public function get_grading_actions() {
        return array('uploadzip'=>get_string('uploadzip', 'assignrecertfeedback_file'));
    }
