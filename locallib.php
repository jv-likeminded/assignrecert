...
/**
 * This file contains the definition for the class assignmentrecert
 *
...
// Assignment Recert submission statuses.
define('ASSIGNRECERT_SUBMISSION_STATUS_NEW', 'new');
define('ASSIGNRECERT_SUBMISSION_STATUS_REOPENED', 'reopened');
define('ASSIGNRECERT_SUBMISSION_STATUS_DRAFT', 'draft');
define('ASSIGNRECERT_SUBMISSION_STATUS_SUBMITTED', 'submitted');

// Search filters for grading page.
define('ASSIGNRECERT_FILTER_SUBMITTED', 'submitted');
define('ASSIGNRECERT_FILTER_NOT_SUBMITTED', 'notsubmitted');
define('ASSIGNRECERT_FILTER_SINGLE_USER', 'singleuser');
define('ASSIGNRECERT_FILTER_REQUIRE_GRADING', 'require_grading');
define('ASSIGNRECERT_FILTER_GRANTED_EXTENSION', 'granted_extension');

// Marker filter for grading page.
define('ASSIGNRECERT_MARKER_FILTER_NO_MARKER', -1);

// Reopen attempt methods.
define('ASSIGNRECERT_ATTEMPT_REOPEN_METHOD_NONE', 'none');
define('ASSIGNRECERT_ATTEMPT_REOPEN_METHOD_MANUAL', 'manual');
define('ASSIGNRECERT_ATTEMPT_REOPEN_METHOD_UNTILPASS', 'untilpass');

// Special value means allow unlimited attempts.
define('ASSIGNRECERT_UNLIMITED_ATTEMPTS', -1);

// Special value means no grade has been set.
define('ASSIGNRECERT_GRADE_NOT_SET', -1);

// Grading states.
define('ASSIGNRECERT_GRADING_STATUS_GRADED', 'graded');
define('ASSIGNRECERT_GRADING_STATUS_NOT_GRADED', 'notgraded');
define('ASSIGNRECERT_GRADING_STATUS_GRADING_NOT_REQUIRED', 'gradingnotrequired');

// Marking workflow states.
define('ASSIGNRECERT_MARKING_WORKFLOW_STATE_NOTMARKED', 'notmarked');
define('ASSIGNRECERT_MARKING_WORKFLOW_STATE_INMARKING', 'inmarking');
define('ASSIGNRECERT_MARKING_WORKFLOW_STATE_READYFORREVIEW', 'readyforreview');
define('ASSIGNRECERT_MARKING_WORKFLOW_STATE_INREVIEW', 'inreview');
define('ASSIGNRECERT_MARKING_WORKFLOW_STATE_READYFORRELEASE', 'readyforrelease');
define('ASSIGNRECERT_MARKING_WORKFLOW_STATE_RELEASED', 'released');

/** ASSIGNRECERT_MAX_EVENT_LENGTH = 432000 ; 5 days maximum */
define("ASSIGNRECERT_MAX_EVENT_LENGTH", "432000");

// Name of file area for intro attachments.
define('ASSIGNRECERT_INTROATTACHMENT_FILEAREA', 'introattachment');
...
require_once($CFG->dirroot . '/mod/assignrecert/renderable.php');
...
/**
 * Standard base class for mod_assignrecert (assignmentrecert types).
 *
 * @package   mod_assignrecert
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class assignrecert {
    ...
    /** @var stdClass the assignmentrecert record that contains the global settings for this assignrecert instance */
    private $instance;

    /** @var grade_item the grade_item record for this assignrecert instance's primary grade item. */
    private $gradeitem;

    /** @var context the context of the course module for this assignrecert instance
     *               (or just the course if we are creating a new one)
     */
    private $context;

    /** @var stdClass the course this assignrecert instance belongs to */
    private $course;

    /** @var stdClass the admin config for all assignrecert instances  */
    private $adminconfig;

    /** @var assignrecert_renderer the custom renderer for this module */
    private $output;

    /** @var cm_info the course module for this assignrecert instance */
    ...
    /** @var array cached list of participants for this assignmentrecert. The cache key will be group, showactive and the context id */
    private $participants = array();

    /** @var array cached list of participants IDs for this assignmentrecert. The cache key will be group, showactive and the context id */
    ...
    /**
     * Constructor for the base assignrecert class.
     *
    ...
    public function __construct($coursemodulecontext, $coursemodule, $course) {
        ...
        $this->submissionplugins = $this->load_plugins('assignrecertsubmission');
        $this->feedbackplugins = $this->load_plugins('assignrecertfeedback');

        // Extra entropy is required for uniqid() to work on cygwin.
        $this->useridlistid = clean_param(uniqid('', true), PARAM_ALPHANUM);

        if (!isset($SESSION->mod_assignrecert_useridlist)) {
            $SESSION->mod_assignrecert_useridlist = [];
        }
    }
    ...
    public function register_return_link($action, $params) {
        ...
        if ($cm) {
            $currenturl = new moodle_url('/mod/assignrecert/view.php', array('id' => $cm->id));
        } else {
            $currenturl = new moodle_url('/mod/assignrecert/index.php', array('id' => $this->get_course()->id));
        }
    ...
    /**
     * Based on the current assignmentrecert settings should we display the intro.
     *
    ...
    /**
     * Does an assignmentrecert have submission(s) or grade(s) already?
     *
     * @return bool
     */
     ...
     *
     * @param string $subtype assignrecertsubmission | assignrecertfeedback
     * @param string $type
     * @return mixed assignrecert_plugin|null
     */
    public function get_plugin_by_type($subtype, $type) {
        $shortsubtype = substr($subtype, strlen('assignrecert'));
     ...
     * @return mixed assignrecert_feedback_plugin|null
     */
    public function get_feedback_plugin_by_type($type) {
        return $this->get_plugin_by_type('assignrecertfeedback', $type);
    }
     ...
     *
     * @param string $type - The type of plugin e.g comments
     * @return mixed assignrecert_submission_plugin|null
     */
    public function get_submission_plugin_by_type($type) {
        return $this->get_plugin_by_type('assignrecertsubmission', $type);
    }
    ...
    public function load_plugins($subtype) {
        ...
        foreach ($names as $name => $path) {
            if (file_exists($path . '/locallib.php')) {
                ...
                $shortsubtype = substr($subtype, strlen('assignrecert'));
                $pluginclass = 'assignrecert_' . $shortsubtype . '_' . $name;

                $plugin = new $pluginclass($this, $name);

                if ($plugin instanceof assignrecert_plugin) {
    ...
    /**
     * Display the assignmentrecert, used by view.php
     *
     * The assignmentrecert is displayed differently depending on your role,
     * the settings for the assignmentrecert and the status of the assignmentrecert.
     *
    ...
    public function view($action='', $args = array()) {
        ...
        if (!empty($action)) {
            $PAGE->set_pagetype('mod-assignrecert-' . $action);
        }
        // Now show the right view page.
        if ($action == 'redirect') {
            $nextpageurl = new moodle_url('/mod/assignrecert/view.php', $nextpageparams);
            redirect($nextpageurl);
            return;
        } else if ($action == 'savegradingresult') {
            $message = get_string('gradingchangessaved', 'assignrecert');
        ...
        } else if ($action == 'viewpluginassignrecertfeedback') {
            $o .= $this->view_plugin_content('assignrecertfeedback');
        } else if ($action == 'viewpluginassignrecertsubmission') {
            $o .= $this->view_plugin_content('assignrecertsubmission');
            ...
            $o .= $this->view_error_page(get_string('submitforgrading', 'assignrecert'), $notices);
     ...
     *             when upgrading an old assignmentrecert to a new one (the plugins get called manually)
    ...
    public function add_instance(stdClass $formdata, $callplugins) {
        ...
        $returnid = $DB->insert_record('assignrecert', $update);
        $this->instance = $DB->get_record('assignrecert', array('id'=>$returnid), '*', MUST_EXIST);
        ...
        $DB->update_record('assignrecert', $update);

        return $returnid;
    }

    /**
     * Delete all grades from the gradebook for this assignmentrecert.
     *
     * @return bool
     */
    protected function delete_grades() {
        ...
        $result = grade_update('mod/assignrecert',
                               $this->get_course()->id,
                               'mod',
                               'assignrecert',
    ...
    public function delete_instance() {
        ...
        // Delete files associated with this assignmentrecert.
        $fs = get_file_storage();
        ...
        $DB->delete_records('assignrecert_submission', array('assignmentrecert' => $this->get_instance()->id));
        $DB->delete_records('assignrecert_grades', array('assignmentrecert' => $this->get_instance()->id));
        $DB->delete_records('assignrecert_plugin_config', array('assignmentrecert' => $this->get_instance()->id));
        $DB->delete_records('assignrecert_user_flags', array('assignmentrecert' => $this->get_instance()->id));
        $DB->delete_records('assignrecert_user_mapping', array('assignmentrecert' => $this->get_instance()->id));
        ...
        // Delete the instance.
        $DB->delete_records('assignrecert', array('id'=>$this->get_instance()->id));

        return $result;
    }

    /**
     * Deletes a assignrecert override from the database and clears any corresponding calendar events
     *
    ...
    public function delete_override($overrideid) {
        ...
        if (empty($cm)) {
            ...
            $cm = get_coursemodule_from_instance('assignrecert', $instance->id, $instance->course);
        }

        $override = $DB->get_record('assignrecert_overrides', array('id' => $overrideid), '*', MUST_EXIST);

        // Delete the events.
        $conds = array('modulename' => 'assignrecert', 'instance' => $this->get_instance()->id);
        ...
        $DB->delete_records('assignrecert_overrides', array('id' => $overrideid));
        ...
        $params = array(
            ...
            'other' => array(
                'assignrecertid' => $override->assignrecertid
            )
        );
        // Determine which override deleted event to fire.
        if (!empty($override->userid)) {
            $params['relateduserid'] = $override->userid;
            $event = \mod_assignrecert\event\user_override_deleted::create($params);
        } else {
            $params['other']['groupid'] = $override->groupid;
            $event = \mod_assignrecert\event\group_override_deleted::create($params);
        }

        // Trigger the override deleted event.
        $event->add_record_snapshot('assignrecert_overrides', $override);
        $event->trigger();
    ...
    /**
     * Deletes all assignrecert overrides from the database and clears any corresponding calendar events
     */
    public function delete_all_overrides() {
        global $DB;

        $overrides = $DB->get_records('assignrecert_overrides', array('assignrecertid' => $this->get_instance()->id), 'id');
    ...
    /**
     * Updates the assignrecert properties with override information for a user.
     *
     * Algorithm:  For each assignrecert setting, if there is a matching user-specific override,
     *   then use that otherwise, if there are group-specific overrides, return the most
     *   lenient combination of them.  If neither applies, leave the assignrecert setting unchanged.
     *
    ...
    public function update_effective_access($userid) {
        ...
        // Merge with assignrecert defaults.
    ...
    /**
     * Returns whether an assignrecert has any overrides.
     *
    ...
    public function has_overrides() {
        ...
        $override = $DB->record_exists('assignrecert_overrides', array('assignrecertid' => $this->get_instance()->id));
     ...
     *   lowest sort order. If neither applies, leave the assignrecert setting unchanged.
     *
     * @param int $userid The userid.
    ...
    public function override_exists($userid) {
        ...
        $getuseroverride = function($userid) use ($DB) {
            $useroverride = $DB->get_record('assignrecert_overrides', ['assignrecertid' => $this->get_instance()->id, 'userid' => $userid]);
        ...
        $getgroupoverride = function($userid) use ($DB) {
            ...
            $sql = "SELECT * FROM {assignrecert_overrides}
                    WHERE groupid $extra AND assignrecertid = ? ORDER BY sortorder ASC";
     ...
     * assignmentrecert submissions for course $data->courseid.
     *
    ...
    public function reset_userdata($data) {
        ...
        $componentstr = get_string('modulenameplural', 'assignrecert');
        $status = array();

        $fs = get_file_storage();
        if (!empty($data->reset_assignrecert_submissions)) {
            // Delete files associated with this assignmentrecert.
            foreach ($this->submissionplugins as $plugin) {
                $fileareas = array();
                ...
                if (!$plugin->delete_instance()) {
                    $status[] = array('component'=>$componentstr,
                                      'item'=>get_string('deleteallsubmissions', 'assignrecert'),
            ...
            $assignrecertids = $DB->get_records('assignrecert', array('course' => $data->courseid), '', 'id');
            list($sql, $params) = $DB->get_in_or_equal(array_keys($assignrecertids));

            $DB->delete_records_select('assignrecert_submission', "assignmentrecert $sql", $params);
            $DB->delete_records_select('assignrecert_user_flags', "assignmentrecert $sql", $params);
            ...
            if (!empty($data->reset_gradebook_grades)) {
                $DB->delete_records_select('assignrecert_grades', "assignmentrecert $sql", $params);
                // Remove all grades from gradebook.
                require_once($CFG->dirroot.'/mod/assignrecert/lib.php');
                assignrecert_reset_gradebook($data->courseid);
            }

            // Reset revealidentities for assignrecert if blindmarking is enabled.
            if ($this->get_instance()->blindmarking) {
                $DB->set_field('assignrecert', 'revealidentities', 0, array('id' => $this->get_instance()->id));
            }
        }

        // Remove user overrides.
        if (!empty($data->reset_assignrecert_user_overrides)) {
            $DB->delete_records_select('assignrecert_overrides',
                'assignrecertid IN (SELECT id FROM {assignrecert} WHERE course = ?) AND userid IS NOT NULL', array($data->courseid));
            $status[] = array(
                'component' => $componentstr,
                'item' => get_string('useroverridesdeleted', 'assignrecert'),
                'error' => false);
        }
        // Remove group overrides.
        if (!empty($data->reset_assignrecert_group_overrides)) {
            $DB->delete_records_select('assignrecert_overrides',
                'assignrecertid IN (SELECT id FROM {assignrecert} WHERE course = ?) AND groupid IS NOT NULL', array($data->courseid));
            $status[] = array(
                'component' => $componentstr,
                'item' => get_string('groupoverridesdeleted', 'assignrecert'),
                'error' => false);
        }

        // Updating dates - shift may be negative too.
        if ($data->timeshift) {
            $DB->execute("UPDATE {assignrecert_overrides}
                         SET allowsubmissionsfromdate = allowsubmissionsfromdate + ?
                       WHERE assignrecertid = ? AND allowsubmissionsfromdate <> 0",
                array($data->timeshift, $this->get_instance()->id));
            $DB->execute("UPDATE {assignrecert_overrides}
                         SET duedate = duedate + ?
                       WHERE assignrecertid = ? AND duedate <> 0",
                array($data->timeshift, $this->get_instance()->id));
            $DB->execute("UPDATE {assignrecert_overrides}
                         SET cutoffdate = cutoffdate + ?
                       WHERE assignrecertid =? AND cutoffdate <> 0",
                array($data->timeshift, $this->get_instance()->id));

            shift_course_mod_dates('assignrecert',
    ...
    public function delete_user_submission($userid) {
        ...
        if ($submission = $this->get_user_submission($userid, false)) {
            ...
            // Delete files associated with this assignmentrecert for this user.
            $assignmentrecertid = $this->get_instance()->id;
            $plugintypes = array();
            ...
            foreach ($plugintypes as $plugintype => $params) {
                $deleteparams = array('assignmentrecert' => $assignmentrecertid, $params['fieldname'] => $params['itemid'] );
            ...
            $DB->delete_records('assignrecert_submission', array('id' => $submission->id));
        }

        // Remove assignmentrecert grade.
        if ($grade) {
            $DB->delete_records('assignrecert_grades', array('id' => $grade->id));
        }
     ...
     *
     * @param assignrecert_plugin $plugin The plugin to update
     * @param stdClass $formdata The form data
     * @return bool false if an error occurs
     */
    protected function update_plugin_instance(assignrecert_plugin $plugin, stdClass $formdata) {
    ...
    /**
     * Update the gradebook information for this assignmentrecert.
     *
     * @param bool $reset If true, will reset all grades in the gradbook for this assignmentrecert
    ...
    public function update_gradebook($reset, $coursemoduleid) {
        ...
        require_once($CFG->dirroot.'/mod/assignrecert/lib.php');
        $assignrecert = clone $this->get_instance();
        $assignrecert->cmidnumber = $coursemoduleid;

        // Set assignrecert gradebook feedback plugin status (enabled and visible).
        $assignrecert->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
        ...
        return assignrecert_grade_item_update($assignrecert, $param);
    }
     ...
     * @return integer
     */
    public function get_assignrecert_perpage() {
        $perpage = (int) get_user_preferences('assignrecert_perpage', 10);
    ...
    public function get_admin_config() {
        ...
        $this->adminconfig = get_config('assignrecert');
        return $this->adminconfig;
    }

    /**
     * Update the calendar entries for this assignmentrecert.
     *
    ...
    public function update_calendar($coursemoduleid) {
        ...
        if ($instance->duedate) {
            ...
            $params = array('modulename' => 'assignrecert', 'instance' => $instance->id, 'eventtype' => $eventtype);
            ...
            $DB->delete_records('event', array('modulename' => 'assignrecert', 'instance' => $instance->id, 'eventtype' => $eventtype));
        }
    }
    ...
    public function update_instance($formdata) {
        ...
        $result = $DB->update_record('assignrecert', $update);
        $this->instance = $DB->get_record('assignrecert', array('id'=>$update->id), '*', MUST_EXIST);

        $this->save_intro_draft_files($formdata);

        // Load the assignmentrecert so the plugins have access to it.
    ...
    protected function save_intro_draft_files($formdata) {
        if (isset($formdata->introattachments)) {
            file_save_draft_area_files($formdata->introattachments, $this->get_context()->id,
                                       'mod_assignrecert', ASSIGNRECERT_INTROATTACHMENT_FILEAREA, 0);
        }
    }
     ...
     *
     * @param assignrecert_plugin $plugin The plugin to add the settings from
     ...
     * @return void
     */
    protected function add_plugin_settings(assignrecert_plugin $plugin, MoodleQuickForm $mform, & $pluginsenabled) {
        global $CFG;
    ...
    public function add_all_plugin_settings(MoodleQuickForm $mform) {
        $mform->addElement('header', 'submissiontypes', get_string('submissiontypes', 'assignrecert'));

        $submissionpluginsenabled = array();
        $group = $mform->addGroup(array(), 'submissionplugins', get_string('submissiontypes', 'assignrecert'), array(' '), false);
        foreach ($this->submissionplugins as $plugin) {
            $this->add_plugin_settings($plugin, $mform, $submissionpluginsenabled);
        }
        $group->setElements($submissionpluginsenabled);

        $mform->addElement('header', 'feedbacktypes', get_string('feedbacktypes', 'assignrecert'));
        $feedbackpluginsenabled = array();
        $group = $mform->addGroup(array(), 'feedbackplugins', get_string('feedbacktypes', 'assignrecert'), array(' '), false);
        foreach ($this->feedbackplugins as $plugin) {
     ...
     *
     * @return string the module name (Assignment Recert)
     */
    protected function get_module_name() {
        if (isset(self::$modulename)) {
            return self::$modulename;
        }
        self::$modulename = get_string('modulename', 'assignrecert');
        return self::$modulename;
    }
     ...
     *
     * @return string the module name plural (Assignment Recerts)
     */
    protected function get_module_name_plural() {
        if (isset(self::$modulenameplural)) {
            return self::$modulenameplural;
        }
        self::$modulenameplural = get_string('modulenameplural', 'assignrecert');
        return self::$modulenameplural;
    }

    /**
     * Has this assignmentrecert been constructed from an instance?
     *
     * @return bool
     */
    ...
    /**
     * Get the settings for the current instance of this assignmentrecert
     *
    ...
    public function get_instance() {
        ...
        if ($this->get_course_module()) {
            ...
            $this->instance = $DB->get_record('assignrecert', $params, '*', MUST_EXIST);
        }
        if (!$this->instance) {
            throw new coding_exception('Improper use of the assignmentrecert class. ' .
                                       'Cannot load the assignmentrecert record.');
        }
        return $this->instance;
    }

    /**
     * Get the primary grade item for this assignrecert instance.
     *
    ...
    protected function count_attachments() {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->get_context()->id, 'mod_assignrecert', ASSIGNRECERT_INTROATTACHMENT_FILEAREA,
                        0, 'id', false);
        ...
        if ($this->get_instance()->grade >= 0) {
            ...
            if ($editing && $this->get_instance()->grade > 0) {
                ...
                $o .= '<label class="accesshide" for="quickgrade_' . $userid . '">' .
                       get_string('usergrade', 'assignrecert') .
                       '</label>';
                $o .= '<input type="text"
    ...
    /**
     * Get the submission status/grading status for all submissions in this assignmentrecert for the
     * given paticipants.
     *
     * These statuses match the available filters (requiregrading, submitted, notsubmitted, grantedextension).
     * If this is a group assignmentrecert, group info is also returned.
     *
    ...
    private function get_submission_info_for_participants($participants) {
        ...
        $assignrecertid = $this->get_instance()->id;
        $params['assignmentid1'] = $assignrecertid;
        $params['assignmentid2'] = $assignrecertid;
        $params['assignmentid3'] = $assignrecertid;
        ...
        $from = ' FROM {user} u
                         LEFT JOIN {assignrecert_submission} s
                                ON u.id = s.userid
                               AND s.assignmentrecert = :assignmentid1
                               AND s.latest = 1
                         LEFT JOIN {assignrecert_grades} g
                                ON u.id = g.userid
                               AND g.assignmentrecert = :assignmentid2
                               AND g.attemptnumber = s.attemptnumber
                         LEFT JOIN {assignrecert_user_flags} uf
                                ON u.id = uf.userid
                               AND uf.assignmentrecert = :assignmentid3
            ';
        $where = ' WHERE u.id ' . $insql;

        if (!empty($this->get_instance()->blindmarking)) {
            $from .= 'LEFT JOIN {assignrecert_user_mapping} um
                             ON u.id = um.userid
                            AND um.assignmentrecert = :assignmentid4 ';
            $params['assignmentid4'] = $assignrecertid;
        ...
        foreach ($records as $userid => $submissioninfo) {
            ...
            if (!empty($submissioninfo->stime) && $submissioninfo->status == ASSIGNRECERT_SUBMISSION_STATUS_SUBMITTED) {
                $submitted = true;
            }
    ...
    /**
     * Get the submission status/grading status for all submissions in this assignmentrecert.
     * These statuses match the available filters (requiregrading, submitted, notsubmitted, grantedextension).
     * If this is a group assignmentrecert, group info is also returned.
     *
    ...
    public function list_participants($currentgroup, $idsonly) {
        ...
        if (!isset($this->participants[$key])) {
            list($esql, $params) = get_enrolled_sql($this->context, 'mod/assignrecert:submit', $currentgroup,
            ...
            if (!empty($instance->blindmarking)) {
                $additionaljoins .= " LEFT JOIN {assignrecert_user_mapping} um
                                  ON u.id = um.userid
                                 AND um.assignmentrecert = :assignmentid1
                           LEFT JOIN {assignrecert_submission} s
                                  ON u.id = s.userid
                                 AND s.assignmentrecert = :assignmentid2
                                 AND s.latest = 1
                        ";
            ...
            if ($instance->markingworkflow &&
                    ...
                    !has_capability('mod/assignrecert:manageallocations', $this->get_context()) &&
                    has_capability('mod/assignrecert:grade', $this->get_context())) {

                $additionaljoins .= ' LEFT JOIN {assignrecert_user_flags} uf
                                     ON u.id = uf.userid
                                     AND uf.assignmentrecert = :assignmentid3';
     ...
     * status for this assignmentrecert.
     *
     * @param int $userid
    ...
    public function get_participant($userid) {
        ...
        if (!is_enrolled($this->context, $participant, 'mod/assignrecert:submit', $this->show_only_active_users())) {
            return null;
        }
    ...
    /**
     * Load a count of valid teams for this assignmentrecert.
     *
    ...
    public function count_submissions_need_grading() {
        ...
        if ($this->get_instance()->teamsubmission) {
            // This does not make sense for group assignmentrecert because the submission is shared.
            return 0;
        }

        $currentgroup = groups_get_activity_group($this->get_course_module(), true);
        list($esql, $params) = get_enrolled_sql($this->get_context(), 'mod/assignrecert:submit', $currentgroup, true);

        $params['assignrecertid'] = $this->get_instance()->id;
        ...
        $sql = 'SELECT COUNT(s.userid)
                   ...
                   WHERE
                        s.latest = 1 AND
                        s.assignmentrecert = :assignrecertid AND
    ...
    public function count_grades() {
        ...
        $sql = 'SELECT COUNT(g.userid)
                   ...
                   WHERE g.assignmentrecert = :assignrecertid';
    ...
    public function count_submissions($includenew = false) {
        ...
        if (!$includenew) {
            ...
            $params['status'] = ASSIGNRECERT_SUBMISSION_STATUS_NEW;
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $currentgroup = groups_get_activity_group($this->get_course_module(), true);
            list($esql, $enrolparams) = get_enrolled_sql($this->get_context(), 'mod/assignrecert:submit', $currentgroup, true);
    ...
    public function count_submissions_with_status($status) {
        ...
        $params['assignrecertid'] = $this->get_instance()->id;
        $params['assignrecertid2'] = $this->get_instance()->id;
    ...
    protected function get_grading_userid_list() {
        $filter = get_user_preferences('assignrecert_filter', '');
        $table = new assignrecert_grading_table($this, 0, $filter, 0, false);
    ...
    protected function pack_files($filesforzipping) {
        ...
        $tempzip = tempnam($CFG->tempdir . '/', 'assignmentrecert_');
        // Zip files.
    ...
    /**
     * Finds all assignmentrecert notifications that have yet to be mailed out, and mails them.
     *
    ...
    public static function cron() {
        ...
        $timenow   = time();
        $lastcron = $DB->get_field('modules', 'lastcron', array('name' => 'assignrecert'));

        // Collect all submissions that require mailing.
        // Submissions are included if all are true:
        //   - The assignmentrecert is visible in the gradebook.
        ...
        $sql = "SELECT g.id as gradeid, a.course, a.name, a.blindmarking, a.revealidentities,
                 ...
                 FROM {assignrecert} a
                 JOIN {assignrecert_grades} g ON g.assignmentrecert = a.id
            LEFT JOIN {assignrecert_user_flags} uf ON uf.assignmentrecert = a.id AND uf.userid = g.userid
                 JOIN {course_modules} cm ON cm.course = a.course AND cm.instance = a.id
                 JOIN {modules} md ON md.id = cm.module AND md.name = 'assignrecert'
                 JOIN {grade_items} gri ON gri.iteminstance = a.id AND gri.courseid = a.course AND gri.itemmodule = md.name
            LEFT JOIN {assignrecert_user_mapping} um ON g.id = um.userid AND um.assignmentrecert = a.id
        ...
        $params = array(
            'yesterday' => $yesterday,
            'today' => $timenow,
            'wfreleased' => ASSIGNRECERT_MARKING_WORKFLOW_STATE_RELEASED,
        );
        $submissions = $DB->get_records_sql($sql, $params);

        if (!empty($submissions)) {

            mtrace('Processing ' . count($submissions) . ' assignmentrecert submissions ...');
            ...
            foreach ($submissions as $submission) {
                ...
                mtrace("Processing assignmentrecert submission $submission->id ...");
                ...
                if (!$cm->uservisible) {
                    // Hold mail notification for assignmentrecerts the user cannot access until later.
                    continue;
                }

                // Need to send this to the student.
                $messagetype = 'feedbackavailable';
                $eventtype = 'assignrecert_notification';
                $updatetime = $submission->lastmodified;
                $modulename = get_string('modulename', 'assignrecert');

                $uniqueid = 0;
                if ($submission->blindmarking && !$submission->revealidentities) {
                    if (empty($submission->recordid)) {
                        $uniqueid = self::get_uniqueid_for_user_static($submission->assignmentrecert, $grader->id);
                    } else {
                        $uniqueid = $submission->recordid;
                    }
                }
                $showusers = $submission->blindmarking && !$submission->revealidentities;
                self::send_assignmentrecert_notification($grader,
                                                   $user,
                                                   $messagetype,
                                                   $eventtype,
                                                   $updatetime,
                ...
                $flags = $DB->get_record('assignrecert_user_flags', array('userid'=>$user->id, 'assignmentrecert'=>$submission->assignmentrecert));
                if ($flags) {
                    $flags->mailed = 1;
                    $DB->update_record('assignrecert_user_flags', $flags);
                } else {
                    $flags = new stdClass();
                    $flags->userid = $user->id;
                    $flags->assignmentrecert = $submission->assignmentrecert;
                    $flags->mailed = 1;
            ...
            mtrace('Done processing ' . count($submissions) . ' assignmentrecert submissions');
        ...
        foreach ($newlyavailable as $record) {
            $cm = get_coursemodule_from_instance('assignrecert', $record->id, 0, false, MUST_EXIST);
            $context = context_module::instance($cm->id);
    ...
    /**
     * Update user flags for this user in this assignmentrecert.
     *
    ...
    public function update_user_flags($flags) {
        global $DB;
        if ($flags->userid <= 0 || $flags->assignmentrecert <= 0 || $flags->id <= 0) {
            return false;
        }

        $result = $DB->update_record('assignrecert_user_flags', $flags);
        return $result;
    }

    /**
     * Update a grade in the grade table for the assignmentrecert and in the gradebook.
     *
     * @param stdClass $grade a grade record keyed on id
     * @param bool $reopenattempt If the attempt reopen method is manual, allow another attempt at this assignmentrecert.
     * @return bool true for success
     */
    public function update_grade($grade, $reopenattempt = false) {
        ...
        $DB->update_record('assignrecert_grades', $grade);
        ...
        if ($this->gradebook_item_update(null, $grade)) {
            \mod_assignrecert\event\submission_graded::create_from_grade($this, $grade)->trigger();
        }
    ...
    protected function view_grant_extension($mform) {
        global $CFG;
        require_once($CFG->dirroot . '/mod/assignrecert/extensionform.php');
        ...
        if (empty($mform)) {
            $mform = new mod_assignrecert_extension_form(null, $formparams);
        }
        $mform->set_data($data);
        $header = new assignrecert_header($this->get_instance(),
                                    $this->get_context(),
                                    $this->show_intro(),
                                    $this->get_course_module()->id,
                                    get_string('grantextension', 'assignrecert'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new assignrecert_form('extensionform', $mform));
        $o .= $this->view_footer();
        return $o;
    }
    ...
    /**
     * Get a list of the users in the same group as this user that have not submitted the assignmentrecert.
     *
     * @param int $groupid The id of the group whose members we want or 0 for the default group
     * @param bool $onlyids Whether to retrieve only the user id's
     * @return array The users (possibly id's only)
        ...
        foreach ($members as $id => $member) {
            ...
            if ($submission && $submission->status == ASSIGNRECERT_SUBMISSION_STATUS_SUBMITTED) {
                ...
                if ($this->is_blind_marking()) {
                    $members[$id]->alias = get_string('hiddenuser', 'assignrecert') .
    ...
    public function get_group_submission($userid, $groupid, $create, $attemptnumber=-1) {
        ...
        $params = array('assignmentrecert'=>$this->get_instance()->id, 'groupid'=>$groupid, 'userid'=>0);
        if ($attemptnumber >= 0) {
        ...
        if ($create) {
            ...
            $submission->assignmentrecert = $this->get_instance()->id;
            $submission->userid = 0;
            ...
            if ($attemptnumber == -1) {
                ...
                $result = $DB->get_records('assignrecert_submission', $params, 'attemptnumber DESC', 'attemptnumber', 0, 1);
                if ($result) {
            ...
            if ($submission->latest) {
                ...
                $DB->set_field('assignrecert_submission', 'latest', 0, $params);
            }
            $submission->status = ASSIGNRECERT_SUBMISSION_STATUS_NEW;
            $sid = $DB->insert_record('assignrecert_submission', $submission);
            return $DB->get_record('assignrecert_submission', array('id' => $sid));
        }
        return false;
    }

    /**
     * View a summary listing of all assignmentrecerts in the current course.
     *
     * @return string
     */
    private function view_course_index() {
        ...
        if (!$cms = get_coursemodules_in_course('assignrecert', $course->id, 'm.duedate')) {
        ...
        $courseindexsummary = new assignrecert_course_index_summary($usesections, $strsectionname);

        $timenow = time();

        $currentsection = '';
        foreach ($modinfo->instances['assignrecert'] as $cm) {
            if (!$cm->uservisible) {
                continue;
            }
            ...
            $timedue = $assignmentrecert->get_instance()->duedate;

            if (has_capability('mod/assignrecert:grade', $context)) {
                $submitted = $assignmentrecert->count_submissions_with_status(ASSIGNRECERT_SUBMISSION_STATUS_SUBMITTED);

            } else if (has_capability('mod/assignrecert:submit', $context)) {
                $usersubmission = $assignmentrecert->get_user_submission($USER->id, false);

                if (!empty($usersubmission->status)) {
                    $submitted = get_string('submissionstatus_' . $usersubmission->status, 'assignrecert');
                } else {
                    $submitted = get_string('submissionstatus_', 'assignrecert');
                }
            }
            $gradinginfo = grade_get_grades($course->id, 'mod', 'assignrecert', $cm->instance, $USER->id);
            ...
            $courseindexsummary->add_assignrecert_info($cm->id, $cm->get_formatted_name(), $sectionname, $timedue, $submitted, $grade);
    ...
    /**
     * This is used for team assignmentrecerts to get the group for the specified user.
    ...
    protected function view_plugin_content($pluginsubtype) {
        ...
        $plugintype = required_param('plugin', PARAM_PLUGIN);
        $item = null;
        if ($pluginsubtype == 'assignrecertsubmission') {
            ...
            $o .= $this->get_renderer()->render(new assignrecert_header($this->get_instance(),
                                                              $this->get_context(),
                                                              $this->show_intro(),
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new assignrecert_submission_plugin_submission($plugin,
                                                              $item,
                                                              assignrecert_submission_plugin_submission::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            // Trigger event for viewing a submission.
            \mod_assignrecert\event\submission_viewed::create_from_submission($this, $item)->trigger();
            ...
            $o .= $this->get_renderer()->render(new assignrecert_header($this->get_instance(),
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new assignrecert_feedback_plugin_feedback($plugin,
                                                              $item,
                                                              assignrecert_feedback_plugin_feedback::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            // Trigger event for viewing feedback.
            \mod_assignrecert\event\feedback_viewed::create_from_grade($this, $item)->trigger();
        }
     ...
     * @param assignrecert_plugin $plugin - The assignmentrecert plugin
     */
    public function download_rewrite_pluginfile_urls($text, $user, $plugin) {
        // The groupname prefix for the urls doesn't depend on the group mode of the assignmentrecert instance.
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            if ($submissiongroup) {
                ...
                $groupname = get_string('defaultteam', 'assignrecert') . '-';
            }
        }

        if ($this->is_blind_marking()) {
            $prefix = $groupname . get_string('participant', 'assignrecert');
        ...
        if (!get_user_preferences('assignrecert_downloadasfolders', 1)) {
    ...
    public function render_editor_content($filearea, $submissionid, $plugintype, $editor, $component, $shortentext = false) {
        ...
        if ($CFG->enableportfolios && has_capability('mod/assignrecert:exportownsubmission', $this->context) && $canexportportfolio) {
            ...
            $portfolioparams = array('cmid' => $this->get_course_module()->id,
                                     ...
                                     'editor' => $editor,
                                     'area'=>$filearea);
            $button->set_callback_options('assignrecert_portfolio_caller', $portfolioparams, 'mod_assignrecert');
            $fs = get_file_storage();
    ...
    protected function view_savegrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new assignrecert_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('savegradingresult', 'assignrecert')));
        $gradingresult = new assignrecert_gradingmessage(get_string('savegradingresult', 'assignrecert'),
                                                   $message,
                                                   $this->get_course_module()->id);
    ...
    protected function view_quickgrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new assignrecert_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('quickgradingresult', 'assignrecert')));
        $lastpage = optional_param('lastpage', null, PARAM_INT);
        $gradingresult = new assignrecert_gradingmessage(get_string('quickgradingresult', 'assignrecert'),
                                                   $message,
                                                   $this->get_course_module()->id,
                                                   false,
                                                   $lastpage);
    ...
    public function require_view_group_submission($groupid) {
        if (!$this->can_view_group_submission($groupid)) {
            throw new required_capability_exception($this->context, 'mod/assignrecert:viewgrades', 'nopermission', '');
        }
    }
    ...
    /**
     * Throw an error if the permissions to view grades in this assignmentrecert are missing.
     *
     * @throws required_capability_exception
     * @return void
     */
    ...
    /**
     * Does this user have view grade or grade permission for this assignmentrecert?
     *
     * @return bool
     */
    public function can_view_grades() {
        // Permissions check.
        if (!has_any_capability(array('mod/assignrecert:viewgrades', 'mod/assignrecert:grade'), $this->context)) {
            return false;
        }
    ...
    /**
     * Does this user have grade permission for this assignmentrecert?
     *
     * @return bool
     */
    public function can_grade() {
        // Permissions check.
        if (!has_capability('mod/assignrecert:grade', $this->context)) {
            return false;
        }
    ...
    /**
     * Download a zip file of all assignmentrecert submissions.
     *
     * @param array $userids Array of user ids to download assignmentrecert submissions in a zip file
    ...
    protected function download_submissions($userids = false) {
        ...
        $students = get_enrolled_users($this->context, "mod/assignrecert:submit", null, 'u.*', null, null, null,
                        $this->show_only_active_users());
        ...
        foreach ($students as $student) {
            ...
            if ((groups_is_member($groupid, $userid) or !$groupmode or !$groupid)) {
                ...
                if ($this->is_blind_marking()) {
                    $prefix = str_replace('_', ' ', $groupname . get_string('participant', 'assignrecert'));
                ...
                if ($submission) {
                    $downloadasfolders = get_user_preferences('assignrecert_downloadasfolders', 1);
                    foreach ($this->submissionplugins as $plugin) {
                        if ($plugin->is_enabled() && $plugin->is_visible()) {
                            if ($downloadasfolders) {
                                // Create a folder for each user for each assignmentrecert plugin.
                                ...
                                // Create a single folder for all users of all assignmentrecert plugins.
        ...
        if (count($filesforzipping) == 0) {
            $header = new assignrecert_header($this->get_instance(),
                                        ...
                                        '',
                                        $this->get_course_module()->id,
                                        get_string('downloadall', 'assignrecert'));
            $result .= $this->get_renderer()->render($header);
            $result .= $this->get_renderer()->notification(get_string('nosubmission', 'assignrecert'));
            $url = new moodle_url('/mod/assignrecert/view.php', array('id'=>$this->get_course_module()->id,
                                                                    'action'=>'grading'));
            $result .= $this->get_renderer()->continue_button($url);
            $result .= $this->view_footer();
        } else if ($zipfile = $this->pack_files($filesforzipping)) {
            \mod_assignrecert\event\all_submissions_downloaded::create_from_assignrecert($this)->trigger();
     ...
     * @param string $url The url to the assignrecert module instance.
    ...
    public function add_to_log($action = '', $info = '', $url='', $return = false) {
        ...
        if ($return) {
            ...
            debugging('The mod_assignrecert add_to_log() function is now deprecated.', DEBUG_DEVELOPER);
            return $args;
        }
    ...
    public function get_renderer() {
        ...
        $this->output = $PAGE->get_renderer('mod_assignrecert', null, RENDERER_TARGET_GENERAL);
        return $this->output;
    }
     ...
     *
     * For team assignmentrecerts there are 2 submissions - the student submission and the team submission
    ...
    public function get_user_submission($userid, $create, $attemptnumber=-1) {
        ...
        $params = array('assignmentrecert'=>$this->get_instance()->id, 'userid'=>$userid, 'groupid'=>0);
        if ($attemptnumber >= 0) {
            $params['attemptnumber'] = $attemptnumber;
        }
    ...
    protected function get_submission($submissionid) {
        ...
        $params = array('assignmentrecert'=>$this->get_instance()->id, 'id'=>$submissionid);
        return $DB->get_record('assignrecert_submission', $params, '*', MUST_EXIST);
    }
    ...
    public function get_user_flags($userid, $create) {
        ...
        $params = array('assignmentrecert'=>$this->get_instance()->id, 'userid'=>$userid);

        $flags = $DB->get_record('assignrecert_user_flags', $params);
        ...
        if ($create) {
            $flags = new stdClass();
            $flags->assignmentrecert = $this->get_instance()->id;
            $flags->userid = $userid;
            $flags->locked = 0;
            ...
            $fid = $DB->insert_record('assignrecert_user_flags', $flags);
            $flags->id = $fid;
            return $flags;
        }
        return false;
    ...
    public function get_user_grade($userid, $create, $attemptnumber=-1) {
        ...
        $grades = $DB->get_records('assignrecert_grades', $params, 'attemptnumber DESC', '*', 0, 1);
        ...
        if ($create) {
            $grade = new stdClass();
            $grade->assignmentrecert   = $this->get_instance()->id;
            $grade->userid       = $userid;
            ...
            $gid = $DB->insert_record('assignrecert_grades', $grade);
            $grade->id = $gid;
            return $grade;
        }
        return false;
    ...
    protected function get_grade($gradeid) {
        ...
        $params = array('assignmentrecert'=>$this->get_instance()->id, 'id'=>$gradeid);
        return $DB->get_record('assignrecert_grades', $params, '*', MUST_EXIST);
    }
    ...
    protected function view_single_grading_panel($args) {
        ...
        // Need submit permission to submit an assignmentrecert.
        require_capability('mod/assignrecert:grade', $this->context);
        ...
        if ($this->can_view_submission($userid)) {
            ...
            $submissionstatus = new assignrecert_submission_status_compact($instance->allowsubmissionsfromdate,
                                                                     ...
                                                                     $this->get_course_module()->id,
                                                                     $this->get_course()->id,
                                                                     assignrecert_submission_status::GRADER_VIEW,
                                                                     $showedit,
                                                                     false,
                                                                     $viewfullnames,
                                                                     $extensionduedate,
        ...
        if ($attemptnumber != -1 && ($attemptnumber + 1) != count($allsubmissions)) {
            ...
            $message = get_string('editingpreviousfeedbackwarning', 'assignrecert', $params);
        ...
        $mform = new mod_assignrecert_grade_form(null,
                                           $formparams,
                                           'post',
                                           '',
        ...
        $o .= $this->get_renderer()->render(new assignrecert_form('gradingform', $mform));

        if (count($allsubmissions) > 1) {
            $allgrades = $this->get_all_grades($userid);
            $history = new assignrecert_attempt_history_chooser($allsubmissions,
                                                          $allgrades,
                                                          $this->get_course_module()->id,
                                                          $userid);
        ...
        \mod_assignrecert\event\grading_form_viewed::create_from_user($this, $user)->trigger();
    ...
    protected function view_single_grade_page($mform) {
        ...
        $header = new assignrecert_header($instance,
                                    $this->get_context(),
                                    false,
                                    $this->get_course_module()->id,
                                    get_string('grading', 'assignrecert'));
        $o .= $this->get_renderer()->render($header);
        ...
        if (!$userid) {
            ...
            if (empty($SESSION->mod_assignrecert_useridlist[$useridlistkey])) {
                $SESSION->mod_assignrecert_useridlist[$useridlistkey] = $this->get_grading_userid_list();
            }
            $useridlist = $SESSION->mod_assignrecert_useridlist[$useridlistkey];
        } else {
            $rownum = 0;
            $useridlistid = 0;
            $useridlist = array($userid);
        ...
        if ($user) {
            $this->update_effective_access($userid);
            $viewfullnames = has_capability('moodle/site:viewfullnames', $this->get_context());
            $usersummary = new assignrecert_user_summary($user,
                                                   $this->get_course()->id,
                                                   $viewfullnames,
                                                   $this->is_blind_marking(),
        ...
        if ($this->can_view_submission($userid)) {
            ...
            $submissionstatus = new assignrecert_submission_status($instance->allowsubmissionsfromdate,
                                                             $instance->alwaysshowdescription,
                                                             $submission,
                                                             $instance->teamsubmission,
                                                             $teamsubmission,
        ...
        if (count($allsubmissions) > 1 && $attemptnumber == -1) {
            $allgrades = $this->get_all_grades($userid);
            $history = new assignrecert_attempt_history($allsubmissions,
                                                  $allgrades,
                                                  $this->get_submission_plugins(),
    ...
    protected function view_reveal_identities_confirm() {
        require_capability('mod/assignrecert:revealidentities', $this->get_context());
        ...
        $urlparams = array('id'=>$this->get_course_module()->id,
                           ...
                           'sesskey'=>sesskey());
        $confirmurl = new moodle_url('/mod/assignrecert/view.php', $urlparams);

        $urlparams = array('id'=>$this->get_course_module()->id,
                           'action'=>'grading');
        $cancelurl = new moodle_url('/mod/assignrecert/view.php', $urlparams);

        $o .= $this->get_renderer()->confirm(get_string('revealidentitiesconfirm', 'assignrecert'),
                                             $confirmurl,
                                             $cancelurl);
        $o .= $this->view_footer();

        \mod_assignrecert\event\reveal_identities_confirmation_page_viewed::create_from_assignrecert($this)->trigger();
    ...
    protected function view_return_links() {
        ...
        $url = new moodle_url('/mod/assignrecert/view.php', $params);
        return $this->get_renderer()->single_button($url, get_string('back'), 'get');
    }

    /**
     * View the grading table of all submissions for this assignmentrecert.
     *
     * @return string
     */
    protected function view_grading_table() {
        ...
        require_once($CFG->dirroot . '/mod/assignrecert/gradingoptionsform.php');
        require_once($CFG->dirroot . '/mod/assignrecert/quickgradingform.php');
        require_once($CFG->dirroot . '/mod/assignrecert/gradingbatchoperationsform.php');
        $o = '';
        ...
        if (has_capability('gradereport/grader:view', $this->get_course_context()) &&
            ...
            $links[$gradebookurl] = get_string('viewgradebook', 'assignrecert');
        }
        if ($this->is_any_submission_plugin_enabled() && $this->count_submissions()) {
            $downloadurl = '/mod/assignrecert/view.php?id=' . $cmid . '&action=downloadall';
            $links[$downloadurl] = get_string('downloadall', 'assignrecert');
        }
        if ($this->is_blind_marking() &&
                has_capability('mod/assignrecert:revealidentities', $this->get_context())) {
            $revealidentitiesurl = '/mod/assignrecert/view.php?id=' . $cmid . '&action=revealidentities';
            $links[$revealidentitiesurl] = get_string('revealidentities', 'assignrecert');
        }
        foreach ($this->get_feedback_plugins() as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                foreach ($plugin->get_grading_actions() as $action => $description) {
                    $url = '/mod/assignrecert/view.php' .
                           '?id=' .  $cmid .
        ...
        $gradingactions->set_label(get_string('choosegradingaction', 'assignrecert'));

        $gradingmanager = get_grading_manager($this->get_context(), 'mod_assignrecert', 'submissions');

        $perpage = $this->get_assignrecert_perpage();
        $filter = get_user_preferences('assignrecert_filter', '');
        $markerfilter = get_user_preferences('assignrecert_markerfilter', '');
        $workflowfilter = get_user_preferences('assignrecert_workflowfilter', '');
        $controller = $gradingmanager->get_active_controller();
        $showquickgrading = empty($controller) && $this->can_grade();
        $quickgrading = get_user_preferences('assignrecert_quickgrading', false);
        $showonlyactiveenrolopt = has_capability('moodle/course:viewsuspendedusers', $this->context);
        $downloadasfolders = get_user_preferences('assignrecert_downloadasfolders', 1);

        $markingallocation = $this->get_instance()->markingworkflow &&
            $this->get_instance()->markingallocation &&
            has_capability('mod/assignrecert:manageallocations', $this->context);
        ...
        if ($markingallocation) {
            ...
            $markers = get_enrolled_users($this->context, 'mod/assignrecert:grade', 0, 'u.*', $sort);
            $markingallocationoptions[''] = get_string('filternone', 'assignrecert');
            $markingallocationoptions[ASSIGNRECERT_MARKER_FILTER_NO_MARKER] = get_string('markerfilternomarker', 'assignrecert');
        ...
        if ($markingworkflow) {
            $notmarked = get_string('markingworkflowstatenotmarked', 'assignrecert');
            $markingworkflowoptions[''] = get_string('filternone', 'assignrecert');
            $markingworkflowoptions[ASSIGNRECERT_MARKING_WORKFLOW_STATE_NOTMARKED] = $notmarked;
        ...
        $gradingoptionsform = new mod_assignrecert_grading_options_form(null,
        ...
        $gradingbatchoperationsform = new mod_assignrecert_grading_batch_operations_form(null,
                                                                                   $batchformparams,
                                                                                   'post',
                                                                                   '',
                                                                                   $classoptions);
        ...
        if ($this->is_blind_marking() && has_capability('mod/assignrecert:viewblinddetails', $this->get_context())) {
            $o .= $this->get_renderer()->notification(get_string('blindmarkingenabledwarning', 'assignrecert'), 'notifymessage');
        }

        // Load and print the table of submissions.
        if ($showquickgrading && $quickgrading) {
            $gradingtable = new assignrecert_grading_table($this, $perpage, $filter, 0, true);
            ...
            $quickformparams = array('cm'=>$this->get_course_module()->id,
                                     ...
                                     'page' => $page);
            $quickgradingform = new mod_assignrecert_quick_grading_form(null, $quickformparams);

            $o .= $this->get_renderer()->render(new assignrecert_form('quickgradingform', $quickgradingform));
        } else {
            $gradingtable = new assignrecert_grading_table($this, $perpage, $filter, 0, false);
        ...
        if ($this->can_grade()) {
            ...
            $SESSION->mod_assignrecert_useridlist[$this->get_useridlist_key()] = $useridlist;
        }
        ...
        if (count($users) != 0 && $this->can_grade()) {
            ...
            $assignform = new assignrecert_form('gradingbatchoperationsform', $gradingbatchoperationsform);
            $o .= $this->get_renderer()->render($assignform);
        }
        $assignform = new assignrecert_form('gradingoptionsform',
                                      $gradingoptionsform,
                                      'M.mod_assignrecert.init_grading_options');
        $o .= $this->get_renderer()->render($assignform);
        return $o;
    }
    ...
    protected function view_grader() {
        ...
        \mod_assignrecert\event\grading_table_viewed::create_from_assignrecert($this)->trigger();
    ...
    /**
     * Message for students when assignmentrecert submissions have been closed.
     *
    ...
    protected function view_notices($title, $notices) {
        ...
        $url = new moodle_url('/mod/assignrecert/view.php', array('id'=>$this->get_course_module()->id, 'action'=>'view'));
    ...
    public function fullname($user) {
        if ($this->is_blind_marking()) {
            $hasviewblind = has_capability('mod/assignrecert:viewblinddetails', $this->get_context());
            ...
            if ($hasviewblind) {
                return get_string('participant', 'assignrecert') . ' ' . $uniqueid . ' (' .
    ...
    protected function view_edit_submission_page($mform, $notices) {
        ...
        $o = '';
        require_once($CFG->dirroot . '/mod/assignrecert/submission_form.php');
        // Need submit permission to submit an assignmentrecert.
        ...
        if ($userid == $USER->id) {
            ...
            require_capability('mod/assignrecert:submit', $this->context);
            $title = get_string('editsubmission', 'assignrecert');
        } else {
            ...
            $title = get_string('editsubmissionother', 'assignrecert', $name);
        }

        if (!$this->submissions_open($userid)) {
            $message = array(get_string('submissionsclosed', 'assignrecert'));
            return $this->view_notices($title, $message);
        }

        $postfix = '';
        if ($this->has_visible_attachments()) {
            $postfix = $this->render_area_files('mod_assignrecert', ASSIGNRECERT_INTROATTACHMENT_FILEAREA, 0);
        }
        $o .= $this->get_renderer()->render(new assignrecert_header($this->get_instance(),
                                                      $this->get_context(),
                                                      $this->show_intro(),
        ...
        $data->userid = $userid;
        if (!$mform) {
            $mform = new mod_assignrecert_submission_form(null, array($this, $data));
        }
        ...
        $o .= $this->get_renderer()->render(new assignrecert_form('editsubmissionform', $mform));

        $o .= $this->view_footer();

        \mod_assignrecert\event\submission_form_viewed::create_from_user($this, $user)->trigger();

        return $o;
    }

    /**
     * Grading not required for this assignmentrecert
     *
     * @return bool
     */
    ...
    /**
     * See if this assignmentrecert has a grade yet.
     *
     * @param int $userid
     * @return bool
     */
    ...
    public function can_view_submission($userid) {
        ...
        if ($userid == $USER->id && has_capability('mod/assignrecert:submit', $this->context)) {
            return true;
        }
        return false;
    }
    ...
    protected function process_grading_batch_operation(& $mform) {
        ...
        $mform = new mod_assignrecert_grading_batch_operations_form(null,
                                                              $batchformparams,
                                                              'post',
                                                              '',
                                                              $formclasses);
    ...
    protected function view_batch_set_workflow_state($mform) {
        ...
        require_once($CFG->dirroot . '/mod/assignrecert/batchsetmarkingworkflowstateform.php');
        ...
        foreach ($userlist as $userid) {
            ...
            $usershtml .= $this->get_renderer()->render(new assignrecert_user_summary($user,
                                                                $this->get_course()->id,
                                                                $viewfullnames,
                                                                $this->is_blind_marking(),
                                                                $this->get_uniqueid_for_user($user->id),
        ...
        $mform = new mod_assignrecert_batch_set_marking_workflow_state_form(null, $formparams);
        $mform->set_data($formdata);    // Initialises the hidden elements.
        $header = new assignrecert_header($this->get_instance(),
            $this->get_context(),
            $this->show_intro(),
            $this->get_course_module()->id,
            get_string('setmarkingworkflowstate', 'assignrecert'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new assignrecert_form('setworkflowstate', $mform));
        $o .= $this->view_footer();

        \mod_assignrecert\event\batch_set_workflow_state_viewed::create_from_assignrecert($this)->trigger();
    ...
    public function view_batch_markingallocation($mform) {
        ...
        require_once($CFG->dirroot . '/mod/assignrecert/batchsetallocatedmarkerform.php');
        ...
        list($sort, $params) = users_order_by_sql('u');
        // Only enrolled users could be assigned as potential markers.
        $markers = get_enrolled_users($this->get_context(), 'mod/assignrecert:grade', 0, 'u.*', $sort);
        $markerlist = array();
        foreach ($markers as $marker) {
        ...
        $mform = new mod_assignrecert_batch_set_allocatedmarker_form(null, $formparams);
        $mform->set_data($formdata);    // Initialises the hidden elements.
        $header = new assignrecert_header($this->get_instance(),
            $this->get_context(),
            $this->show_intro(),
            $this->get_course_module()->id,
            get_string('setmarkingallocation', 'assignrecert'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new assignrecert_form('setworkflowstate', $mform));
        $o .= $this->view_footer();

        \mod_assignrecert\event\batch_set_marker_allocation_viewed::create_from_assignrecert($this)->trigger();
    ...
    protected function check_submit_for_grading($mform) {
        ...
        require_once($CFG->dirroot . '/mod/assignrecert/submissionconfirmform.php');
        ...
        if ($mform == null) {
            $mform = new mod_assignrecert_confirm_submission_form(null, array($requiresubmissionstatement,
                                                                        $submissionstatement,
                                                                        $this->get_course_module()->id,
                                                                        $data));
        }
        ...
        $o .= $this->get_renderer()->render(new assignrecert_header($this->get_instance(),
                                                              ...
                                                              $this->show_intro(),
                                                              $this->get_course_module()->id,
                                                              get_string('confirmsubmissionheading', 'assignrecert')));
        $submitforgradingpage = new assignrecert_submit_for_grading_page($notifications,
                                                                   $this->get_course_module()->id,
                                                                   $mform);
        $o .= $this->get_renderer()->render($submitforgradingpage);
        $o .= $this->view_footer();

        \mod_assignrecert\event\submission_confirmation_form_viewed::create_from_assignrecert($this)->trigger();

        return $o;
    }

    /**
     * Creates an assignrecert_submission_status renderable.
     *
     * @param stdClass $user the user to get the report for
     * @param bool $showlinks return plain text or links to the profile
     * @return assignrecert_submission_status renderable object
     */
    public function get_assignrecert_submission_status_renderable($user, $showlinks) {
        global $PAGE;
        ...
        $gradingmanager = get_grading_manager($this->context, 'mod_assignrecert', 'submissions');
        ...
        $submissionstatus = new assignrecert_submission_status($instance->allowsubmissionsfromdate,
                                                          ...
                                                          $this->get_course_module()->id,
                                                          $this->get_course()->id,
                                                          assignrecert_submission_status::STUDENT_VIEW,
                                                          $showedit,
                                                          $showsubmit,
                                                          $viewfullnames,
                                                          $extensionduedate,
    ...
    /**
     * Creates an assignrecert_feedback_status renderable.
     *
     * @param stdClass $user the user to get the report for
     * @return assignrecert_feedback_status renderable object
     */
    public function get_assignrecert_feedback_status_renderable($user) {
        global $CFG, $DB, $PAGE;
        ...
        if ($this->get_instance()->markingworkflow && $gradingstatus != ASSIGNRECERT_MARKING_WORKFLOW_STATE_RELEASED) {
            $emptyplugins = true; // Don't show feedback plugins until released either.
        }

        $cangrade = has_capability('mod/assignrecert:grade', $this->get_context());
        ...
        if (($hasgrade || !$emptyplugins) && $gradevisible) {
            ...
            $feedbackstatus = new assignrecert_feedback_status($gradefordisplay,
                                                  $gradeddate,
                                                  $grader,
                                                  $this->get_feedback_plugins(),
                                                  $grade,
    ...
    /**
     * Creates an assignrecert_attempt_history renderable.
     *
     * @param stdClass $user the user to get the report for
     * @return assignrecert_attempt_history renderable object
     */
    public function get_assignrecert_attempt_history_renderable($user) {
    ...
    public function view_student_summary($user, $showlinks) {
        ...
        if ($this->can_view_submission($user->id)) {
            ...
            if (has_capability('mod/assignrecert:submit', $this->get_context(), $user, false)) {
                $submissionstatus = $this->get_assignrecert_submission_status_renderable($user, $showlinks);
                $o .= $this->get_renderer()->render($submissionstatus);
            }

            // If there is a visible grade, show the feedback.
            $feedbackstatus = $this->get_assignrecert_feedback_status_renderable($user);
            if ($feedbackstatus) {
                $o .= $this->get_renderer()->render($feedbackstatus);
            }

            // If there is more than one submission, show the history.
            $history = $this->get_assignrecert_attempt_history_renderable($user);
    ...
    protected function show_submit_button($submission = null, $teamsubmission = null, $userid = null) {
        if ($teamsubmission) {
            if ($teamsubmission->status === ASSIGNRECERT_SUBMISSION_STATUS_SUBMITTED) {
                // The assignmentrecert submission has been completed.
                return false;
            } else if ($this->submission_empty($teamsubmission)) {
                // There is nothing to submit yet.
                return false;
            } else if ($submission && $submission->status === ASSIGNRECERT_SUBMISSION_STATUS_SUBMITTED) {
            ...
            if ($submission->status === ASSIGNRECERT_SUBMISSION_STATUS_SUBMITTED) {
                // The assignmentrecert submission has been completed.
                return false;
    ...
    protected function get_all_grades($userid) {
        ...
        $grades = $DB->get_records('assignrecert_grades', $params, 'attemptnumber ASC');
    ...
    protected function get_all_submissions($userid) {
        ...
        $submissions = $DB->get_records('assignrecert_submission', $params, 'attemptnumber ASC');

        return $submissions;
    }

    /**
     * Creates an assignrecert_grading_summary renderable.
     *
     * @return assignrecert_grading_summary renderable object
     */
    public function get_assignrecert_grading_summary_renderable() {

        $instance = $this->get_instance();

        $draft = ASSIGNRECERT_SUBMISSION_STATUS_DRAFT;
        $submitted = ASSIGNRECERT_SUBMISSION_STATUS_SUBMITTED;
        ...
        if ($instance->teamsubmission) {
            ...
            $summary = new assignrecert_grading_summary($this->count_teams($activitygroup),
            ...
            $summary = new assignrecert_grading_summary($countparticipants,
                                                  $instance->submissiondrafts,
                                                  $this->count_submissions_with_status($draft),
                                                  $this->is_any_submission_plugin_enabled(),
                                                  $this->count_submissions_with_status($submitted),
    ...
    protected function view_submission_page() {
        ...
        $o .= $this->get_renderer()->render(new assignrecert_header($instance,
                                                      $this->get_context(),
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      '', '', $postfix));
        ...
        foreach ($plugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                $o .= $this->get_renderer()->render(new assignrecert_plugin_header($plugin));
            }
        }

        if ($this->can_view_grades()) {
            // Group selector will only be displayed if necessary.
            $currenturl = new moodle_url('/mod/assignrecert/view.php', array('id' => $this->get_course_module()->id));
            $o .= groups_print_activity_menu($this->get_course_module(), $currenturl->out(), true);

            $summary = $this->get_assignrecert_grading_summary_renderable();
        ...
        \mod_assignrecert\event\submission_status_viewed::create_from_assignrecert($this)->trigger();
    ...
    protected function gradebook_item_update($submission=null, $grade=null) {
        ...
        if ($this->get_instance()->markingworkflow && !empty($grade) &&
                $this->get_grading_status($grade->userid) != ASSIGNRECERT_MARKING_WORKFLOW_STATE_RELEASED) {
        ...
        $assignrecert = clone $this->get_instance();
        $assignrecert->cmidnumber = $this->get_course_module()->idnumber;
        // Set assignrecert gradebook feedback plugin status (enabled and visible).
        $assignrecert->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
        return assignrecert_grade_item_update($assignrecert, $gradebookgrade) == GRADE_UPDATE_OK;
    }
    ...
    protected function update_team_submission(stdClass $submission, $userid, $updatetime) {
        ...
        // Now check the team settings to see if this assignmentrecert qualifies as submitted or draft.
        $team = $this->get_submission_group_members($submission->groupid, true);

        $allsubmitted = true;
        $anysubmitted = false;
        $result = true;
        if ($submission->status != ASSIGNRECERT_SUBMISSION_STATUS_REOPENED) {
            foreach ($team as $member) {
                ...
                if (!$membersubmission || $membersubmission->status != ASSIGNRECERT_SUBMISSION_STATUS_SUBMITTED
            ...
            if ($this->get_instance()->requireallteammemberssubmit) {
                ...
                $result = $DB->update_record('assignrecert_submission', $submission);
            } else {
                if ($anysubmitted) {
                    $submission->status = ASSIGNRECERT_SUBMISSION_STATUS_SUBMITTED;
                } else {
            ...
            foreach ($team as $member) {
                ...
                $membersubmission->status = ASSIGNRECERT_SUBMISSION_STATUS_REOPENED;
                $result = $DB->update_record('assignrecert_submission', $membersubmission) && $result;
            }
            $result = $DB->update_record('assignrecert_submission', $submission) && $result;
        }
    ...
    /**
     * Is this assignmentrecert open for submissions?
     *
     * Check the due date,
     * prevent late submissions,
     * has this person already submitted,
     * is the assignmentrecert locked?
     *
    ...
    public function submissions_open($userid = 0,
        ...
        if ($submission) {
            ...
            if ($this->get_instance()->submissiondrafts && $submission->status == ASSIGNRECERT_SUBMISSION_STATUS_SUBMITTED) {
                // Drafts are tracked and the student has submitted the assignmentrecert.
                return false;
            }
        }
    ...
    public function render_area_files($component, $area, $submissionid) {
        ...
        return $this->get_renderer()->assignrecert_files($this->context, $submissionid, $area, $component);
    ...
    public function can_edit_submission($userid, $graderid = 0) {
        ...
        if ($userid == $graderid &&
                ...
                has_capability('mod/assignrecert:submit', $this->context, $graderid)) {
            // User can edit their own submission.
            return true;
        }

        if (!has_capability('mod/assignrecert:editothersubmission', $this->context, $graderid)) {
            return false;
        }
    ...
    protected function get_graders($userid) {
        ...
        $potentialgraders = get_enrolled_users($this->context, "mod/assignrecert:grade", null, 'u.*', null, null, null, true);
    ...
    protected function get_notifiable_users($userid) {
        ...
        $potentialusers = get_enrolled_users($this->context, "mod/assignrecert:receivegradernotifications",
    ...
    protected static function format_notification_message_text($messagetype,
        ...
        $posttext .= '---------------------------------------------------------------------' . "\n";
        $posttext .= get_string($messagetype . 'text', 'assignrecert', $info)."\n";
        $posttext .= "\n---------------------------------------------------------------------\n";
        return $posttext;
    }
    ...
    protected static function format_notification_message_html($messagetype,
        ...
        $posthtml  = '<p><font face="sans-serif">' .
                     ...
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/assignrecert/index.php?id=' . $course->id . '">' .
                     $modulename .
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/assignrecert/view.php?id=' . $coursemodule->id . '">' .
                     format_string($assignmentname, true, $formatparams) .
                     '</a></font></p>';
        $posthtml .= '<hr /><font face="sans-serif">';
        $posthtml .= '<p>' . get_string($messagetype . 'html', 'assignrecert', $info) . '</p>';
        $posthtml .= '</font><hr />';
        return $posthtml;
    }
     ...
     * @return void
     */
    public static function send_assignmentrecert_notification($userfrom,
                                                        $userto,
                                                        $messagetype,
                                                        $eventtype,
                                                        $updatetime,
        ...
        if ($blindmarking) {
            ...
            $info->username = get_string('participant', 'assignrecert') . ' ' . $uniqueidforuser;
            $userfrom->firstname = get_string('participant', 'assignrecert');
            $userfrom->lastname = $uniqueidforuser;
            $userfrom->email = $CFG->noreplyaddress;
        } else {
            $info->username = fullname($userfrom, true);
        }
        $info->assignmentrecert = format_string($assignmentname, true, array('context'=>$context));
        $info->url = $CFG->wwwroot.'/mod/assignrecert/view.php?id='.$coursemodule->id;
        $info->timeupdated = userdate($updatetime, get_string('strftimerecentfull'));

        $postsubject = get_string($messagetype . 'small', 'assignrecert', $info);
        ...
        $eventdata->modulename       = 'assignrecert';
        ...
        $eventdata->component       = 'mod_assignrecert';
        $eventdata->notification    = 1;
        $eventdata->contexturl      = $info->url;
        $eventdata->contexturlname  = $info->assignmentrecert;
    ...
    public function send_notification($userfrom, $userto, $messagetype, $eventtype, $updatetime) {
        ...
        self::send_assignmentrecert_notification($userfrom,
                                           $userto,
                                           $messagetype,
                                           $eventtype,
                                           $updatetime,
    ...
    public function submit_for_grading($data, $notices) {
        ...
        if (!$this->submissions_open($userid)) {
            $notices[] = get_string('submissionsclosed', 'assignrecert');
            return false;
        }
        ...
        if ($submission->status != ASSIGNRECERT_SUBMISSION_STATUS_SUBMITTED) {
            ...
            if (!empty($data->submissionstatement) && $USER->id == $userid) {
                \mod_assignrecert\event\statement_accepted::create_from_submission($this, $submission)->trigger();
            }
            $this->notify_graders($submission);
            $this->notify_student_submission_receipt($submission);

            \mod_assignrecert\event\assessable_submitted::create_from_submission($this, $submission, false)->trigger();
    ...
    /**
     * Assignment Recert submission is processed before grading.
     *
    ...
    public function save_user_extension($userid, $extensionduedate) {
        ...
        // Need submit permission to submit an assignmentrecert.
        require_capability('mod/assignrecert:grantextension', $this->context);

        if (!is_enrolled($this->get_course_context(), $userid)) {
            return false;
        }
        if (!has_capability('mod/assignrecert:submit', $this->context, $userid)) {
            return false;
        }
        ...
        if ($result) {
            \mod_assignrecert\event\extension_granted::create_from_assignrecert($this, $userid)->trigger();
        }
        return $result;
    }
    ...
    protected function process_save_quick_grades() {
        ...
        $controller = $gradingmanager->get_active_controller();
        if (!empty($controller)) {
            return get_string('errorquickgradingvsadvancedgrading', 'assignrecert');
        }
        ...
        $sql = 'SELECT u.id AS userid, g.grade AS grade, g.timemodified AS lastmodified,
             ...
             LEFT JOIN {assignrecert_grades} g ON
                       u.id = g.userid AND
                       g.assignmentrecert = :assignrecertid2 AND
                       g.attemptnumber = gmx.maxattempt
             LEFT JOIN {assignrecert_user_flags} uf ON uf.assignmentrecert = g.assignmentrecert AND uf.userid = g.userid
                 WHERE u.id ' . $userids;
                ...
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if ($plugin->is_quickgrading_modified($modified->userid, $grade)) {
                        if ((int)$current->lastmodified > (int)$modified->lastmodified) {
                            return get_string('errorrecordmodified', 'assignrecert');
                        } else {
                            $modifiedusers[$modified->userid] = $modified;
                            continue;
                        }
        ...
        foreach ($modifiedusers as $userid => $modified) {
            ...
            foreach ($this->feedbackplugins as $plugin) {
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if (('assignrecertfeedback_' . $plugin->get_type()) == $gradebookplugin) {
            ...
            if ($workflowstatemodified || $allocatedmarkermodified) {
                if ($this->update_user_flags($flags) && $workflowstatemodified) {
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_assignrecert\event\workflow_state_updated::create_from_user($this, $user, $flags->workflowstate)->trigger();
                }
            }
            ...
            if ($CFG->enableoutcomes) {
                ...
                if (count($data) > 0) {
                    grade_update_outcomes('mod/assignrecert',
                                          $this->course->id,
                                          'mod',
                                          'assignrecert',
                                          $this->get_instance()->id,
        ...
        return get_string('quickgradingchangessaved', 'assignrecert');
    }
    ...
    public function reveal_identities() {
        ...
        require_capability('mod/assignrecert:revealidentities', $this->context);

        if ($this->get_instance()->revealidentities || empty($this->get_instance()->blindmarking)) {
            return false;
        }

        // Update the assignmentrecert record.
        $update = new stdClass();
        $update->id = $this->get_instance()->id;
        ...
        $grades = $DB->get_records('assignrecert_grades', array('assignmentrecert'=>$this->get_instance()->id));
        ...
        \mod_assignrecert\event\identities_revealed::create_from_assignrecert($this)->trigger();
    }
    ...
    protected function process_save_grading_options() {
        ...
        $gradingoptionsparams = array('cm'=>$this->get_course_module()->id,
                                      ...
                                      'showonlyactiveenrol' => $this->show_only_active_users(),
                                      'downloadasfolders' => get_user_preferences('assignrecert_downloadasfolders', 1));
        $mform = new mod_assignrecert_grading_options_form(null, $gradingoptionsparams);
        if ($formdata = $mform->get_data()) {
            set_user_preference('assignrecert_perpage', $formdata->perpage);
            if (isset($formdata->filter)) {
                set_user_preference('assignrecert_filter', $formdata->filter);
            }
            if (isset($formdata->markerfilter)) {
                set_user_preference('assignrecert_markerfilter', $formdata->markerfilter);
            }
            if (isset($formdata->workflowfilter)) {
                set_user_preference('assignrecert_workflowfilter', $formdata->workflowfilter);
            }
            if ($showquickgrading) {
                set_user_preference('assignrecert_quickgrading', isset($formdata->quickgrading));
            }
            if (isset($formdata->downloadasfolders)) {
                set_user_preference('assignrecert_downloadasfolders', 1); // Enabled.
            } else {
                set_user_preference('assignrecert_downloadasfolders', 0); // Disabled.
            }
    ...
    public function format_grade_for_log(stdClass $grade) {
        ...
        $info = get_string('gradestudent', 'assignrecert', array('id'=>$user->id, 'fullname'=>fullname($user)));
        if ($grade->grade != '') {
            $info .= get_string('grade') . ': ' . $this->display_grade($grade->grade, false) . '. ';
        } else {
            $info .= get_string('nograde', 'assignrecert');
        }
        return $info;
    }
    ...
    public function format_submission_for_log(stdClass $submission) {
        ...
        if ($submission->userid) {
            ...
            if ($group) {
                ...
                $name = get_string('defaultteam', 'assignrecert');
            }
        }
        $status = get_string('submissionstatus_' . $submission->status, 'assignrecert');
        $params = array('id'=>$submission->userid, 'fullname'=>$name, 'status'=>$status);
        $info .= get_string('submissionlog', 'assignrecert', $params) . ' <br>';
    ...
    /**
     * Copy the current assignmentrecert submission from the last submitted attempt.
     *
     * @param  array $notices Any error messages that should be shown
     *                        to the user at the top of the edit submission form.
     * @return bool
    ...
    public function copy_previous_attempt(&$notices) {
        ...
        if (!$submission || $submission->status != ASSIGNRECERT_SUBMISSION_STATUS_REOPENED) {
            $notices[] = get_string('submissionnotcopiedinvalidstatus', 'assignrecert');
            return false;
        }
        $flags = $this->get_user_flags($USER->id, false);
        ...
        if ($flags && $flags->locked) {
            $notices[] = get_string('submissionslocked', 'assignrecert');
            return false;
        }
        ...
        \mod_assignrecert\event\submission_duplicated::create_from_submission($this, $submission)->trigger();
        ...
        if (!$instance->submissiondrafts) {
            ...
            // but then they would wonder why there are submitted assignmentrecerts
            // and they haven't been notified.
            \mod_assignrecert\event\assessable_submitted::create_from_submission($this, $submission, true)->trigger();
        }
        return true;
    }
    ...
    /**
     * Save assignmentrecert submission for the current user.
     *
     * @param  stdClass $data
     * @param  array $notices Any error messages that should be shown
     *                        to the user.
    ...
    public function save_submission(stdClass $data, & $notices) {
        ...
        if ($this->new_submission_empty($data)) {
            $notices[] = get_string('submissionempty', 'mod_assignrecert');
            return false;
        }
        ...
        if (isset($data->lastmodified) && ($submission->timemodified > $data->lastmodified)) {
            ...
            if ($submission->status !== ASSIGNRECERT_SUBMISSION_STATUS_NEW) {
                $notices[] = $instance->teamsubmission ? get_string('submissionmodifiedgroup', 'mod_assignrecert')
                                                       : get_string('submissionmodified', 'mod_assignrecert');
                return false;
            }
        }
        ...
        // Get the flags to check if it is locked.
        if ($flags && $flags->locked) {
            print_error('submissionslocked', 'assignrecert');
            return true;
        }
    ...
    /**
     * Save assignmentrecert submission.
     *
     * @param  moodleform $mform
     * @param  array $notices Any error messages that should be shown
     *                        to the user at the top of the edit submission form.
     ...
     * This is specific to the assignmentrecert, marker and student.
     *
     * @param int $userid - The student userid
    ...
    public function add_grade_form_elements(MoodleQuickForm $mform, stdClass $data, $params) {
        ...
        if ($gradinginstance) {
            ...
            if ($this->get_instance()->grade > 0) {
                $name = get_string('gradeoutof', 'assignrecert', $this->get_instance()->grade);
                if (!$gradingdisabled) {
                    $gradingelement = $mform->addElement('text', 'grade', $name);
                    $mform->addHelpButton('grade', 'gradeoutofhelp', 'assignrecert');
                    $mform->setType('grade', PARAM_RAW);
                } else {
                    $strgradelocked = get_string('gradelocked', 'assignrecert');
                    $mform->addElement('static', 'gradedisabled', $name, $strgradelocked);
                    $mform->addHelpButton('gradedisabled', 'gradeoutofhelp', 'assignrecert');
                }
            } else {
        ...
        if ($this->get_instance()->markingworkflow) {
            ...
            $options = array('' => get_string('markingworkflowstatenotmarked', 'assignrecert')) + $states;
            $mform->addElement('select', 'workflowstate', get_string('markingworkflowstate', 'assignrecert'), $options);
            $mform->addHelpButton('workflowstate', 'markingworkflowstate', 'assignrecert');
        }
        ...
        if ($this->get_instance()->markingworkflow &&
            ...
            // Only enrolled users could be assigned as potential markers.
            $markers = get_enrolled_users($this->context, 'mod/assignrecert:grade', 0, 'u.*', $sort);
            $markerlist = array('' =>  get_string('choosemarker', 'assignrecert'));
            $viewfullnames = has_capability('moodle/site:viewfullnames', $this->context);
            foreach ($markers as $marker) {
                $markerlist[$marker->id] = fullname($marker, $viewfullnames);
            }
            $mform->addElement('select', 'allocatedmarker', get_string('allocatedmarker', 'assignrecert'), $markerlist);
            $mform->addHelpButton('allocatedmarker', 'allocatedmarker', 'assignrecert');
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', ASSIGNRECERT_MARKING_WORKFLOW_STATE_READYFORREVIEW);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', ASSIGNRECERT_MARKING_WORKFLOW_STATE_INREVIEW);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', ASSIGNRECERT_MARKING_WORKFLOW_STATE_READYFORRELEASE);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', ASSIGNRECERT_MARKING_WORKFLOW_STATE_RELEASED);
        }
        $gradestring = '<span class="currentgrade">' . $gradestring . '</span>';
        $mform->addElement('static', 'currentgrade', get_string('currentgrade', 'assignrecert'), $gradestring);

        if (count($useridlist) > 1) {
            $strparams = array('current'=>$rownum+1, 'total'=>count($useridlist));
            $name = get_string('outof', 'assignrecert', $strparams);
            $mform->addElement('static', 'gradingstudent', get_string('gradingstudent', 'assignrecert'), $name);
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $mform->addElement('selectyesno', 'applytoall', get_string('applytoteam', 'assignrecert'));
        ...
        if (($attemptnumber == -1 ||
            ...
            $this->get_instance()->attemptreopenmethod != ASSIGNRECERT_ATTEMPT_REOPEN_METHOD_NONE) {
            $mform->addElement('header', 'attemptsettings', get_string('attemptsettings', 'assignrecert'));
            $attemptreopenmethod = get_string('attemptreopenmethod_' . $this->get_instance()->attemptreopenmethod, 'assignrecert');
            $mform->addElement('static', 'attemptreopenmethod', get_string('attemptreopenmethod', 'assignrecert'), $attemptreopenmethod);
            ...
            if ($maxattempts == ASSIGNRECERT_UNLIMITED_ATTEMPTS) {
                $maxattempts = get_string('unlimitedattempts', 'assignrecert');
            }
            $mform->addelement('static', 'maxattemptslabel', get_string('maxattempts', 'assignrecert'), $maxattempts);
            $mform->addelement('static', 'attemptnumberlabel', get_string('attemptnumber', 'assignrecert'), $attemptnumber + 1);

            $ismanual = $this->get_instance()->attemptreopenmethod == ASSIGNRECERT_ATTEMPT_REOPEN_METHOD_MANUAL;
            $issubmission = !empty($submission);
            $isunlimited = $this->get_instance()->maxattempts == ASSIGNRECERT_UNLIMITED_ATTEMPTS;
            ...
            if ($ismanual && (!$issubmission || $isunlimited || $islessthanmaxattempts)) {
                $mform->addElement('selectyesno', 'addattempt', get_string('addattempt', 'assignrecert'));
        ...
        if (!$gradingpanel) {
            ...
            $mform->addElement('hidden', 'sendstudentnotifications', get_string('sendstudentnotifications', 'assignrecert'));
            $mform->setType('sendstudentnotifications', PARAM_BOOL);
        }
        // Get assignmentrecert visibility information for student.
        $modinfo = get_fast_modinfo($settings->course, $userid);
        $cm = $modinfo->get_cm($this->get_course_module()->id);

        // Don't allow notification to be sent if the student can't access the assignmentrecert,
        ...
        if (!$cm->uservisible) {
            ...
            $mform->setDefault('sendstudentnotifications', 0);
            if (!$gradingpanel) {
                $mform->disabledIf('sendstudentnotifications', 'workflowstate', 'neq', ASSIGNRECERT_MARKING_WORKFLOW_STATE_RELEASED);
            }
        } else {
        ...
        if (!$gradingpanel) {
            ...
            $buttonarray = array();
            $name = get_string('savechanges', 'assignrecert');
            $buttonarray[] = $mform->createElement('submit', 'savegrade', $name);
            if (!$last) {
                $name = get_string('savenext', 'assignrecert');
            ...
            if ($rownum > 0) {
                $name = get_string('previous', 'assignrecert');
                $buttonarray[] = $mform->createElement('submit', 'nosaveandprevious', $name);
            }

            if (!$last) {
                $name = get_string('nosavebutnext', 'assignrecert');
    ...
    public function revert_to_draft($userid) {
        ...
        \mod_assignrecert\event\submission_status_updated::create_from_submission($this, $submission)->trigger();
        return true;
    }
    ...
    public function lock_submission($userid) {
        ...
        $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
        \mod_assignrecert\event\submission_locked::create_from_user($this, $user)->trigger();
        return true;
    }
    ...
    protected function process_set_batch_marking_workflow_state() {
        ...
        if ($formdata = $mform->get_data()) {
            ...
            foreach ($useridlist as $userid) {
                ...
                // notified previously, the student can access the assignmentrecert, and the state
                // is "Released".
                $modinfo = get_fast_modinfo($this->course, $userid);
                $cm = $modinfo->get_cm($this->get_course_module()->id);
                if ($formdata->sendstudentnotifications && $cm->uservisible &&
                        $state == ASSIGNRECERT_MARKING_WORKFLOW_STATE_RELEASED) {
                    $flags->mailed = 0;
                }

                $gradingdisabled = $this->grading_disabled($userid);

                // Will not apply update if user does not have permission to assignrecert this workflow state.
                if (!$gradingdisabled && $this->update_user_flags($flags)) {
                    if ($state == ASSIGNRECERT_MARKING_WORKFLOW_STATE_RELEASED) {
                        // Update Gradebook.
                        $assignrecert = clone $this->get_instance();
                        $assignrecert->cmidnumber = $this->get_course_module()->idnumber;
                        // Set assignrecert gradebook feedback plugin status.
                        $assignrecert->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
                        assignrecert_update_grades($assignrecert, $userid);
                    }

                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_assignrecert\event\workflow_state_updated::create_from_user($this, $user, $state)->trigger();
                }
            }
        }
    }
    ...
    protected function process_set_batch_marking_allocation() {
        ...
        if ($formdata = $mform->get_data()) {
            ...
            foreach ($useridlist as $userid) {
                $flags = $this->get_user_flags($userid, true);
                if ($flags->workflowstate == ASSIGNRECERT_MARKING_WORKFLOW_STATE_READYFORREVIEW ||
                    $flags->workflowstate == ASSIGNRECERT_MARKING_WORKFLOW_STATE_INREVIEW ||
                    $flags->workflowstate == ASSIGNRECERT_MARKING_WORKFLOW_STATE_READYFORRELEASE ||
                    $flags->workflowstate == ASSIGNRECERT_MARKING_WORKFLOW_STATE_RELEASED) {
                ...
                if ($this->update_user_flags($flags)) {
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_assignrecert\event\marker_updated::create_from_marker($this, $user, $marker)->trigger();
                }
            }
        }
    }
    ...
    public function unlock_submission($userid) {
        ...
        $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
        \mod_assignrecert\event\submission_unlocked::create_from_user($this, $user)->trigger();
        return true;
    }
    ...
    protected function apply_grade_to_user($formdata, $userid, $attemptnumber) {
        ...
        if (!$gradingdisabled) {
            ...
            if (isset($formdata->workflowstate) || isset($formdata->allocatedmarker)) {
                ...
                if ($this->update_user_flags($flags) &&
                    ...
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_assignrecert\event\workflow_state_updated::create_from_user($this, $user, $formdata->workflowstate)->trigger();
                }
            }
        }
        $grade->grader= $USER->id;
        ...
        if ($this->get_instance()->markingworkflow &&
                isset($formdata->workflowstate) &&
                $formdata->workflowstate != ASSIGNRECERT_MARKING_WORKFLOW_STATE_RELEASED) {
    ...
    protected function reopen_submission_if_required($userid, $submission, $addattempt) {
        ...
        $maxattemptsreached = !empty($submission) &&
                              ...
                              $instance->maxattempts != ASSIGNRECERT_UNLIMITED_ATTEMPTS;
        $shouldreopen = false;
        if ($instance->attemptreopenmethod == ASSIGNRECERT_ATTEMPT_REOPEN_METHOD_UNTILPASS) {
        ...
        if ($instance->attemptreopenmethod == ASSIGNRECERT_ATTEMPT_REOPEN_METHOD_MANUAL &&
                !empty($addattempt)) {
            $shouldreopen = true;
        }
    ...
    protected function process_save_grade(&$mform) {
        ...
        $userid = optional_param('userid', 0, PARAM_INT);
        if (!$userid) {
            if (empty($SESSION->mod_assignrecert_useridlist[$this->get_useridlist_key($useridlistid)])) {
                ...
                $url = new moodle_url('/mod/assignrecert/view.php', array('id' => $this->get_course_module()->id));
                throw new moodle_exception('useridlistnotcached', 'mod_assignrecert', $url);
            }
            $useridlist = $SESSION->mod_assignrecert_useridlist[$this->get_useridlist_key($useridlistid)];
        } else {
            $useridlist = array($userid);
            $rownum = 0;
        }
    ...
    public static function can_upgrade_assignment($type, $version) {
        ...
        $assignmentrecert = new assignrecert(null, null, null);
        return $assignmentrecert->can_upgrade($type, $version);
    }

    /**
     * This function returns true if it can upgrade an assignmentrecert from the 2.2 module.
     *
     * @deprecated since Totara 13
     * @param string $type The plugin type
     * @param int $version The plugin version
    ...
    /**
     * Copy all the files from the old assignmentrecert files area to the new one.
     * This is used by the plugin upgrade code.
     *
     * @param int $oldcontextid The old assignmentrecert context id
     * @param int $oldcomponent The old assignmentrecert component ('assignmentrecert')
     * @param int $oldfilearea The old assignmentrecert filearea ('submissions')
     * @param int $olditemid The old submissionid (can be null e.g. intro)
     * @param int $newcontextid The new assignmentrecert context id
     * @param int $newcomponent The new assignmentrecert component ('assignmentrecert')
     * @param int $newfilearea The new assignmentrecert filearea ('submissions')
    ...
    /**
     * Add a new attempt for each user in the list - but reopen each group assignmentrecert
     * at most 1 time.
     *
    ...
    protected function add_attempt($userid) {
        ...
        if ($this->get_instance()->attemptreopenmethod == ASSIGNRECERT_ATTEMPT_REOPEN_METHOD_NONE) {
            return false;
        }
        ...
        if ($this->get_instance()->maxattempts != ASSIGNRECERT_UNLIMITED_ATTEMPTS &&
        ...
        $newsubmission->status = ASSIGNRECERT_SUBMISSION_STATUS_REOPENED;
    ...
    public function get_user_grades_for_gradebook($userid) {
        ...
        foreach ($this->feedbackplugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                if (('assignrecertfeedback_' . $plugin->get_type()) == $gradebookpluginname) {
        ...
        $params = array('assignrecertid1'=>$assignmentrecertid,
                        'assignrecertid2'=>$assignmentrecertid,
                        'userid'=>$userid);
        $graderesults = $DB->get_recordset_sql('SELECT
                                                ...
                                                FROM {user} u
                                                LEFT JOIN {assignrecert_submission} s
                                                    ON u.id = s.userid and s.assignmentrecert = :assignrecertid1 AND
                                                    s.latest = 1
                                                JOIN {assignrecert_grades} g
                                                    ON u.id = g.userid and g.assignmentrecert = :assignrecertid2 AND
                                                    g.attemptnumber = s.attemptnumber' .
                                                $where, $params);
    ...
    /**
     * Foreach participant in the course - assignrecert them a random id.
     *
     * @param int $assignrecertid The assignrecertid to lookup
     */
    public static function allocate_unique_ids($assignrecertid) {
        global $DB;

        $cm = get_coursemodule_from_instance('assignrecert', $assignrecertid, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);

        $currentgroup = groups_get_activity_group($cm, true);
        $users = get_enrolled_users($context, "mod/assignrecert:submit", $currentgroup, 'u.id');

        // Shuffle the users.
        shuffle($users);

        foreach ($users as $user) {
            $record = $DB->get_record('assignrecert_user_mapping',
                                      array('assignmentrecert'=>$assignrecertid, 'userid'=>$user->id),
                                     'id');
            if (!$record) {
                $record = new stdClass();
                $record->assignmentrecert = $assignrecertid;
                $record->userid = $user->id;
                $DB->insert_record('assignrecert_user_mapping', $record);
            }
        }
    }

    /**
     * Lookup this user id and return the unique id for this assignmentrecert.
     *
     * @param int $assignrecertid The assignmentrecert id
     * @param int $userid The userid to lookup
     * @return int The unique id
     */
    public static function get_uniqueid_for_user_static($assignrecertid, $userid) {
        global $DB;

        // Search for a record.
        $params = array('assignmentrecert'=>$assignrecertid, 'userid'=>$userid);
        if ($record = $DB->get_record('assignrecert_user_mapping', $params, 'id')) {
            return $record->id;
        }
        ...
        self::allocate_unique_ids($assignrecertid);
        ...
        return $DB->insert_record('assignrecert_user_mapping', $record);
    }
    ...
    /**
     * Lookup this unique id and return the user id for this assignmentrecert.
     *
     * @param int $assignrecertid The id of the assignmentrecert this user mapping is in
     * @param int $uniqueid The uniqueid to lookup
     * @return int The user id or false if they don't exist
     */
    public static function get_user_id_for_uniqueid_static($assignrecertid, $uniqueid) {
        global $DB;

        // Search for a record.
        if ($record = $DB->get_record('assignrecert_user_mapping',
                                      array('assignmentrecert'=>$assignrecertid, 'id'=>$uniqueid),
                                      'userid',
                                      IGNORE_MISSING)) {
    ...
    public function get_marking_workflow_states_for_current_user() {
        ...
        $states = array();
        if (has_capability('mod/assignrecert:grade', $this->context)) {
            $states[ASSIGNRECERT_MARKING_WORKFLOW_STATE_INMARKING] = get_string('markingworkflowstateinmarking', 'assignrecert');
            $states[ASSIGNRECERT_MARKING_WORKFLOW_STATE_READYFORREVIEW] = get_string('markingworkflowstatereadyforreview', 'assignrecert');
        }
        if (has_any_capability(array('mod/assignrecert:reviewgrades',
                                     'mod/assignrecert:managegrades'), $this->context)) {
            $states[ASSIGNRECERT_MARKING_WORKFLOW_STATE_INREVIEW] = get_string('markingworkflowstateinreview', 'assignrecert');
            $states[ASSIGNRECERT_MARKING_WORKFLOW_STATE_READYFORRELEASE] = get_string('markingworkflowstatereadyforrelease', 'assignrecert');
        }
        if (has_any_capability(array('mod/assignrecert:releasegrades',
                                     'mod/assignrecert:managegrades'), $this->context)) {
            $states[ASSIGNRECERT_MARKING_WORKFLOW_STATE_RELEASED] = get_string('markingworkflowstatereleased', 'assignrecert');
        }
    ...
    public function get_grading_status($userid) {
        if ($this->get_instance()->markingworkflow) {
            ...
            return ASSIGNRECERT_MARKING_WORKFLOW_STATE_NOTMARKED;
        } else {

            if ($this->grading_not_required()) {
                return ASSIGNRECERT_GRADING_STATUS_GRADING_NOT_REQUIRED;
            }
            ...
            if (!empty($grade) && $grade->grade !== null && $grade->grade >= 0) {
                return ASSIGNRECERT_GRADING_STATUS_GRADED;
            } else {
                return ASSIGNRECERT_GRADING_STATUS_NOT_GRADED;
            }
        }
    }

    /**
     * The id used to uniquily identify the cache for this instance of the assignrecert object.
     *
     * @return string
     */
    ...
    /**
     * Checks for any grade notices, and adds notifications. Will display on assignmentrecert main page and grading table.
     *
    ...
    protected function add_grade_notices() {
        if (has_capability('mod/assignrecert:grade', $this->get_context()) && get_config('assignrecert', 'has_rescaled_null_grades_' . $this->get_instance()->id)) {
            $link = new \moodle_url('/mod/assignrecert/view.php', array('id' => $this->get_course_module()->id, 'action' => 'fixrescalednullgrades'));
            \core\notification::warning(get_string('fixrescalednullgrades', 'mod_assignrecert', ['link' => $link->out()]));
        }
    }
    ...
    protected function view_fix_rescaled_null_grades() {
        ...
        require_capability('mod/assignrecert:grade', $this->get_context());

        $instance = $this->get_instance();

        $o .= $this->get_renderer()->render(
            new assignrecert_header(
                $instance,
                $this->get_context(),
                $this->show_intro(),
                $this->get_course_module()->id
        ...
        if ($confirm) {
            ...
            unset_config('has_rescaled_null_grades_' . $instance->id, 'assignrecert');

            // Display the notice.
            $o .= $this->get_renderer()->notification(get_string('fixrescalednullgradesdone', 'assignrecert'), 'notifysuccess');
            $url = new moodle_url(
                '/mod/assignrecert/view.php',
                array(
            ...
            $continue = new \moodle_url('/mod/assignrecert/view.php', array('id' => $this->get_course_module()->id, 'action' => 'fixrescalednullgrades', 'confirm' => true, 'sesskey' => sesskey()));
            $cancel = new \moodle_url('/mod/assignrecert/view.php', array('id' => $this->get_course_module()->id));
            $o .= $OUTPUT->confirm(get_string('fixrescalednullgradesconfirm', 'mod_assignrecert'), $continue, $cancel);
        }
     ...
     * attempt on a group assignmentrecert, and whether the gradebook should be updated.
     *
     * @since Moodle 3.2.6
    ...
    private function get_instance_from_form($formdata) {
        ...
        $instance->attemptreopenmethod = ASSIGNRECERT_ATTEMPT_REOPEN_METHOD_NONE;
...
/**
 * Portfolio caller class for mod_assignrecert.
 *
 * @package   mod_assignrecert
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignrecert_portfolio_caller extends portfolio_module_caller_base {
    ...
    /** @var int callback arg - the cmid of the assignmentrecert we export */
    protected $cmid;
    ...
    public function __construct($callbackargs) {
        ...
        $this->cm = get_coursemodule_from_id('assignrecert', $this->cmid, 0, false, MUST_EXIST);
    }

    /**
     * Load data needed for the portfolio export.
     *
     * If the assignmentrecert type implements portfolio_load_data(), the processing is delegated
    ...
    public function load_data() {
        ...
        if (empty($this->fileid)) {
            if (empty($this->sid) || empty($this->area)) {
                throw new portfolio_caller_exception('invalidfileandsubmissionid', 'mod_assignrecert');
            }

            $submission = $DB->get_record('assignrecert_submission', array('id' => $this->sid));
        } else {
    ...
    public function prepare_package() {
        ...
        if ($this->plugin && $this->editor) {
            ...
            if (in_array($exporterclass, array(PORTFOLIO_FORMAT_PLAINHTML, PORTFOLIO_FORMAT_RICHHTML))) {
                ...
                return $this->exporter->write_new_file($html, 'assignmentrecert.html', !empty($files));
        ...
        if ($this->exporter->get('formatclass') == PORTFOLIO_FORMAT_LEAP2A) {
            ...
            if (count($files) > 1) {
                $baseid = 'assignrecert' . $this->cmid . $this->area;
                $context = context_module::instance($this->cmid);
     ...
     *
     * @return assignrecert_submission_plugin
     */
    protected function get_submission_plugin() {
        ...
        $assignmentrecert = new assignrecert($context, null, null);
        return $assignmentrecert->get_submission_plugin_by_type($this->plugin);
    }
    ...
    public function check_permissions() {
        $context = context_module::instance($this->cmid);
        return has_capability('mod/assignrecert:exportownsubmission', $context);
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'assignrecert');
    }
 ...
 * @return void
 */
function assignrecert_process_group_deleted_in_course($courseid, $groupid = null) {
    global $DB;
    ...
    if ($groupid) {
        ...
        $sql = "SELECT o.id, o.assignrecertid
                  FROM {assignrecert_overrides} o
                  JOIN {assignrecert} assignrecert ON assignrecert.id = o.assignrecertid
                 WHERE assignrecert.course = :courseid
                   AND o.groupid = :groupid";
    } else {
        // No groupid, we update all orphaned group overrides for all assignrecert in course.
        $sql = "SELECT o.id, o.assignrecertid
                  FROM {assignrecert_overrides} o
                  JOIN {assignrecert} assignrecert ON assignrecert.id = o.assignrecertid
    ...
    $DB->delete_records_list('assignrecert_overrides', 'id', array_keys($records));
}
 ...
 * @param int $assignrecertid of the assignmentrecert
 * @return bool success of operation
 */
function move_group_override($id, $move, $assignrecertid) {
    global $DB;

    // Get the override object.
    if (!$override = $DB->get_record('assignrecert_overrides', array('id' => $id), 'id, sortorder')) {
        return false;
    }
    // Count the number of group overrides.
    $overridecountgroup = $DB->count_records('assignrecert_overrides', array('userid' => null, 'assignrecertid' => $assignrecertid));
    ...
    $params = array('sortorder' => $neworder, 'assignrecertid' => $assignrecertid);
    if ($swapoverride = $DB->get_record('assignrecert_overrides', $params, 'id, sortorder')) {
        ...
        $DB->update_record('assignrecert_overrides', $override);
        $DB->update_record('assignrecert_overrides', $swapoverride);
    }

    reorder_group_overrides($assignrecertid);
    return true;
}
 ...
 *
 * @param int $assignrecertid of the assigment
 */
function reorder_group_overrides($assignrecertid) {
    global $DB;

    $i = 1;
    if ($overrides = $DB->get_records('assignrecert_overrides', array('userid' => null, 'assignrecertid' => $assignrecertid), 'sortorder ASC')) {
        foreach ($overrides as $override) {
            $f = new stdClass();
            $f->id = $override->id;
            $f->sortorder = $i++;
            $DB->update_record('assignrecert_overrides', $f);
        }
    }
}
