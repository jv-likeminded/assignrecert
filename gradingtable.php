...
/**
 * Extends table_sql to provide a table of assignmentrecert submissions
 *
 * @package   mod_assignrecert
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class assignrecert_grading_table extends table_sql implements renderable {
    /** @var assignrecert $assignmentrecert */
    private $assignmentrecert = null;
    /** @var int $perpage */
    private $perpage = 10;
    ...
    /**
     * overridden constructor keeps a reference to the assignmentrecert class that is displaying this table
     *
     ...
     */
    public function __construct(assignrecert $assignmentrecert,
                                $perpage,
                                $filter,
                                $rowoffset,
                                $quickgrading,
        ...
        parent::__construct('mod_assignrecert_grading');
        $this->is_persistent(true);
        $this->assignmentrecert = $assignmentrecert;

        // Check permissions up front.
        $this->hasgrantextension = has_capability('mod/assignrecert:grantextension',
                                                  $this->assignmentrecert->get_context());
        $this->hasgrade = $this->assignmentrecert->can_grade();

        // Check if we have the elevated view capablities to see the blind details.
        $this->hasviewblind = has_capability('mod/assignrecert:viewblinddetails',
                $this->assignmentrecert->get_context());
        ...
        $this->output = $PAGE->get_renderer('mod_assignrecert');

        $urlparams = array('action'=>'grading', 'id'=>$assignmentrecert->get_course_module()->id);
        $url = new moodle_url($CFG->wwwroot . '/mod/assignrecert/view.php', $urlparams);
        ...
        $users = array_keys($assignmentrecert->list_participants($currentgroup, true));
        if (count($users) == 0) {
        ...
        $params = array();
        $params['assignmentid1'] = (int)$this->assignmentrecert->get_instance()->id;
        $params['assignmentid2'] = (int)$this->assignmentrecert->get_instance()->id;
        $params['assignmentid3'] = (int)$this->assignmentrecert->get_instance()->id;
        $params['newstatus'] = ASSIGNRECERT_SUBMISSION_STATUS_NEW;

        $extrauserfields = get_extra_user_fields($this->assignmentrecert->get_context());
        ...
        // For group assignmentrecerts, there can be a grade with no submission.
        $from .= ' LEFT JOIN {assignrecert_grades} g
                            ON g.assignmentrecert = :assignmentid2
                           AND u.id = g.userid
                           AND (g.attemptnumber = s.attemptnumber OR s.attemptnumber IS NULL) ';

        $from .= 'LEFT JOIN {assignrecert_user_flags} uf
                         ON u.id = uf.userid
                        AND uf.assignmentrecert = :assignmentid3 ';

        $hasoverrides = $this->assignmentrecert->has_overrides();

        if ($hasoverrides) {
            $params['assignmentid5'] = (int)$this->assignmentrecert->get_instance()->id;
            $params['assignmentid6'] = (int)$this->assignmentrecert->get_instance()->id;
            $params['assignmentid7'] = (int)$this->assignmentrecert->get_instance()->id;
            $params['assignmentid8'] = (int)$this->assignmentrecert->get_instance()->id;
            $params['assignmentid9'] = (int)$this->assignmentrecert->get_instance()->id;
            ...
            $from .= ' LEFT JOIN (
               SELECT merged.userid, min(merged.priority) priority FROM (
                  ...
                  ( SELECT uo.userid, 0 AS priority
                      FROM {assignrecert_overrides} uo
                     WHERE uo.assignrecertid = :assignmentid5
                  )
                  UNION
                  ( SELECT gm.userid, go.sortorder AS priority
                      FROM {assignrecert_overrides} go
                      JOIN {groups} g ON g.id = go.groupid
                      JOIN {groups_members} gm ON gm.groupid = g.id
                     WHERE go.assignrecertid = :assignmentid6
                  )
                ) merged
            ...
            JOIN (
              (SELECT 9999999 AS priority,
                      ...
                      a.cutoffdate
                 FROM {user} u
                 JOIN {assignrecert} a ON a.id = :assignmentid7
              )
              UNION
              (SELECT 0 AS priority,
                      ...
                      uo.cutoffdate
                 FROM {assignrecert_overrides} uo
                WHERE uo.assignrecertid = :assignmentid8
              )
              UNION
              (SELECT go.sortorder AS priority,
                 ...
                 JOIN {groups} g ON g.id = go.groupid
                 JOIN {groups_members} gm ON gm.groupid = g.id
                WHERE go.assignrecertid = :assignmentid9
              )

            ) effective ON effective.priority = priority.priority AND effective.userid = priority.userid ';
        }

        if (!empty($this->assignmentrecert->get_instance()->blindmarking)) {
            $from .= 'LEFT JOIN {assignrecert_user_mapping} um
                             ON u.id = um.userid
                            AND um.assignmentrecert = :assignmentidblind ';
            $params['assignmentidblind'] = (int)$this->assignmentrecert->get_instance()->id;
        ...
        if ($this->assignmentrecert->is_any_submission_plugin_enabled()) {
            if ($filter == ASSIGNRECERT_FILTER_SUBMITTED) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 s.status = :submitted) ';
                $params['submitted'] = ASSIGNRECERT_SUBMISSION_STATUS_SUBMITTED;

            } else if ($filter == ASSIGNRECERT_FILTER_NOT_SUBMITTED) {
                $where .= ' AND (s.timemodified IS NULL OR s.status <> :submitted) ';
                $params['submitted'] = ASSIGNRECERT_SUBMISSION_STATUS_SUBMITTED;
            } else if ($filter == ASSIGNRECERT_FILTER_REQUIRE_GRADING) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 s.status = :submitted AND
                                 (s.timemodified >= g.timemodified OR g.timemodified IS NULL OR g.grade IS NULL';

                if ($this->assignmentrecert->get_grade_item()->gradetype == GRADE_TYPE_SCALE) {
            ...
            } else if ($filter == ASSIGNRECERT_FILTER_GRANTED_EXTENSION) {
                $where .= ' AND uf.extensionduedate > 0 ';

            } else if (strpos($filter, ASSIGNRECERT_FILTER_SINGLE_USER) === 0) {
        ...
        if ($this->assignmentrecert->get_instance()->markingworkflow &&
            $this->assignmentrecert->get_instance()->markingallocation) {
            if (has_capability('mod/assignrecert:manageallocations', $this->assignmentrecert->get_context())) {
                // Check to see if marker filter is set.
                $markerfilter = (int)get_user_preferences('assignrecert_markerfilter', '');
                if (!empty($markerfilter)) {
                    if ($markerfilter == ASSIGNRECERT_MARKER_FILTER_NO_MARKER) {
        ...
        if ($this->assignmentrecert->get_instance()->markingworkflow) {
            $workflowstates = $this->assignmentrecert->get_marking_workflow_states_for_current_user();
            if (!empty($workflowstates)) {
                $workflowfilter = get_user_preferences('assignrecert_workflowfilter', '');
                if ($workflowfilter == ASSIGNRECERT_MARKING_WORKFLOW_STATE_NOTMARKED) {
                    $where .= ' AND (uf.workflowstate = :workflowstate OR uf.workflowstate IS NULL OR '.
                        $DB->sql_isempty('assignrecert_user_flags', 'workflowstate', true, true).')';
        ...
        // User picture.
        if ($this->hasviewblind || !$this->assignmentrecert->is_blind_marking()) {
            if (!$this->is_downloading()) {
                $columns[] = 'picture';
                $headers[] = get_string('pictureofuser');
            } else {
                $columns[] = 'recordid';
                $headers[] = get_string('recordid', 'assignrecert');
            }
            ...
            if ($this->assignmentrecert->is_blind_marking()) {
                if (!$this->is_downloading()) {
                    $columns[] = 'recordid';
                    $headers[] = get_string('recordid', 'assignrecert');
                }
        ...
        // Submission status.
        $columns[] = 'status';
        $headers[] = get_string('status', 'assignrecert');

        if ($hasoverrides) {
            // Allowsubmissionsfromdate.
            $columns[] = 'allowsubmissionsfromdate';
            $headers[] = get_string('allowsubmissionsfromdate', 'assignrecert');

            // Duedate.
            $columns[] = 'duedate';
            $headers[] = get_string('duedate', 'assignrecert');

            // Cutoffdate.
            $columns[] = 'cutoffdate';
            $headers[] = get_string('cutoffdate', 'assignrecert');
        }

        // Team submission columns.
        if ($assignmentrecert->get_instance()->teamsubmission) {
            $columns[] = 'team';
            $headers[] = get_string('submissionteam', 'assignrecert');
        }
        // Allocated marker.
        if ($this->assignmentrecert->get_instance()->markingworkflow &&
            $this->assignmentrecert->get_instance()->markingallocation &&
            has_capability('mod/assignrecert:manageallocations', $this->assignmentrecert->get_context())) {
            // Add a column for the allocated marker.
            $columns[] = 'allocatedmarker';
            $headers[] = get_string('marker', 'assignrecert');
        }
        // Grade.
        $columns[] = 'grade';
        ...
        if ($this->is_downloading()) {
            if ($this->assignmentrecert->get_instance()->grade >= 0) {
                $columns[] = 'grademax';
                $headers[] = get_string('maxgrade', 'assignrecert');
            } else {
                // This is a custom scale.
                $columns[] = 'scale';
                $headers[] = get_string('scale', 'assignrecert');
            }

            if ($this->assignmentrecert->get_instance()->markingworkflow) {
                // Add a column for the marking workflow state.
                $columns[] = 'workflowstate';
                $headers[] = get_string('markingworkflowstate', 'assignrecert');
            }
            // Add a column for the list of valid marking workflow states.
            $columns[] = 'gradecanbechanged';
            $headers[] = get_string('gradecanbechanged', 'assignrecert');
        }
        if (!$this->is_downloading() && $this->hasgrade) {
        ...
        // Submission plugins.
        if ($assignmentrecert->is_any_submission_plugin_enabled()) {
            $columns[] = 'timesubmitted';
            $headers[] = get_string('lastmodifiedsubmission', 'assignrecert');
        ...
        // Time marked.
        $columns[] = 'timemarked';
        $headers[] = get_string('lastmodifiedgrade', 'assignrecert');

        // Feedback plugins.
        foreach ($this->assignmentrecert->get_feedback_plugins() as $plugin) {
            if ($this->is_downloading()) {
                if ($plugin->is_visible() && $plugin->is_enabled()) {
                    foreach ($plugin->get_editor_fields() as $field => $description) {
                        $index = 'plugin' . count($this->plugincache);
    ...
    /**
     * Add a column with an ID that uniquely identifies this user in this assignmentrecert.
     *
     * @param stdClass $row
     * @return string
     */
    public function col_recordid(stdClass $row) {
        if (empty($row->recordid)) {
            $row->recordid = $this->assignmentrecert->get_uniqueid_for_user($row->userid);
        }
        return get_string('hiddenuser', 'assignrecert') . $row->recordid;
    }
    ...
    public function col_workflowstatus(stdClass $row) {
        ...
        $gradingdisabled = $this->assignmentrecert->grading_disabled($row->id);
        // The function in the assignmentrecert keeps a static cache of this list of states.
        $workflowstates = $this->assignmentrecert->get_marking_workflow_states_for_current_user();
        ...
        if ($this->quickgrading && !$gradingdisabled) {
            ...
            $o .= $this->output->container(get_string('markingworkflowstate' . $workflowstate, 'assignrecert'), $workflowstate);
        }
        return $o;
    }
    ...
    public function col_workflowstate($row) {
        ...
        if (empty($state)) {
            $state = ASSIGNRECERT_MARKING_WORKFLOW_STATE_NOTMARKED;
        }

        return get_string('markingworkflowstate' . $state, 'assignrecert');
    }
    ...
    public function col_allocatedmarker(stdClass $row) {
        ...
        if ($markers === null) {
            ...
            $markers = get_enrolled_users($this->assignmentrecert->get_context(), 'mod/assignrecert:grade', 0, 'u.*', $sort);
            $markerlist[0] = get_string('choosemarker', 'assignrecert');
            $viewfullnames = has_capability('moodle/site:viewfullnames', $this->assignmentrecert->get_context());
        ...
        if ($this->is_downloading()) {
            if (isset($markers[$row->allocatedmarker])) {
                return fullname($markers[$row->allocatedmarker],
                        has_capability('moodle/site:viewfullnames', $this->assignmentrecert->get_context()));
            } else {
                return '';
            }
        }

        if ($this->quickgrading && has_capability('mod/assignrecert:manageallocations', $this->assignmentrecert->get_context()) &&
            (empty($row->workflowstate) ||
             $row->workflowstate == ASSIGNRECERT_MARKING_WORKFLOW_STATE_INMARKING ||
             $row->workflowstate == ASSIGNRECERT_MARKING_WORKFLOW_STATE_NOTMARKED)) {
    ...
    public function col_scale($row) {
        ...
        if (empty($this->scale)) {
            $dbparams = array('id'=>-($this->assignmentrecert->get_instance()->grade));
    ...
    public function display_grade($grade, $editable, $userid, $modified) {
        if ($this->is_downloading()) {
            if ($this->assignmentrecert->get_instance()->grade >= 0) {
                ...
                $gradeitem = $this->assignmentrecert->get_grade_item();
                return format_float($grade, $gradeitem->get_decimals());
            } else {
                // This is a custom scale.
                $scale = $this->assignmentrecert->display_grade($grade, false);
                if ($scale == '-') {
                    $scale = '';
                }
                return $scale;
        ...
        return $this->assignmentrecert->display_grade($grade, $editable, $userid, $modified);
    }
    ...
    public function col_team(stdClass $row) {
        ...
        if ($group) {
            return $group->name;
        } else if ($this->assignmentrecert->get_instance()->preventsubmissionnotingroup) {
            $usergroups = $this->assignmentrecert->get_all_groups($row->id);
            if (count($usergroups) > 1) {
                return get_string('multipleteamsgrader', 'assignrecert');
            } else {
                return get_string('noteamgrader', 'assignrecert');
            }
        }
        return get_string('defaultteam', 'assignrecert');
    }
    ...
    protected function get_group_and_submission($userid, &$group, &$submission, $attemptnumber) {
        ...
        if (isset($this->submissiongroups[$userid])) {
            ...
            $group = $this->assignmentrecert->get_submission_group($userid, false);
        ...
        if (isset($this->groupsubmissions[$groupid . ':' . $attemptnumber])) {
            ...
            $submission = $this->assignmentrecert->get_group_submission($userid, $groupid, false, $attemptnumber);
    ...
    public function col_outcomes(stdClass $row) {
        ...
        $grades = grade_get_grades($this->assignmentrecert->get_course()->id,
            'mod',
            'assignrecert',
            $this->assignmentrecert->get_instance()->id,
    ...
    public function col_fullname($row) {
        if (!$this->is_downloading()) {
            ...
            $fullname = $this->output->action_link($link, $this->assignmentrecert->fullname($row));
        } else {
            $fullname = $this->assignmentrecert->fullname($row);
        }

        if (!$this->assignmentrecert->is_active_user($row->id)) {
    ...
    public function col_select(stdClass $row) {
        ...
        $selectcol .= get_string('selectuser', 'assignrecert', $this->assignmentrecert->fullname($row));
        $selectcol .= '</label>';
    ...
    /**
     * Return a users grades from the listing of all grade data for this assignmentrecert.
     *
     * @param int $userid
    ...
    private function get_gradebook_data_for_user($userid) {
        if (isset($this->rawdata[$userid])) {
            ...
            $grade = grade_get_grades($this->assignmentrecert->get_course()->id,
                'mod',
                'assignrecert',
                $this->assignmentrecert->get_instance()->id,
    ...
    public function col_grademax(stdClass $row) {
        $gradeitem = $this->assignmentrecert->get_grade_item();
        return format_float($this->assignmentrecert->get_instance()->grade, $gradeitem->get_decimals());
    }
    ...
    public function col_grade(stdClass $row) {
        ...
        if (!$this->is_downloading() && $this->hasgrade) {
            $urlparams = array('id' => $this->assignmentrecert->get_course_module()->id,
                               'rownum' => 0,
                               'action' => 'grader');
            ...
            $str = $this->assignmentrecert->grading_not_required() ? 'review' : 'grade';
            $link = '<a href="' . $url . '" class="btn btn-default">' . get_string($str, 'assignrecert') . '</a>';
    ...
    public function col_timesubmitted(stdClass $row) {
        ...
        $submission = false;
        $this->get_group_and_submission($row->id, $group, $submission, -1);
        if ($submission && $submission->timemodified && $submission->status != ASSIGNRECERT_SUBMISSION_STATUS_NEW) {
            $o = userdate($submission->timemodified);
        } else if ($row->timesubmitted && $row->status != ASSIGNRECERT_SUBMISSION_STATUS_NEW) {
    ...
    public function col_status(stdClass $row) {
        ...
        $instance = $this->assignmentrecert->get_instance();
        ...
        if ($this->assignmentrecert->is_any_submission_plugin_enabled()) {
            ...
            $o .= $this->output->container(get_string('submissionstatus_' . $displaystatus, 'assignrecert'),
                                           array('class'=>'submissionstatus' .$displaystatus));
            if ($due && $timesubmitted > $due && $status != ASSIGNRECERT_SUBMISSION_STATUS_NEW) {
            ...
            if ($row->locked) {
                $lockedstr = get_string('submissionslockedshort', 'assignrecert');
            ...
            if (!$instance->markingworkflow) {
                if ($row->grade !== null && $row->grade >= 0) {
                    $o .= $this->output->container(get_string('graded', 'assignrecert'), 'submissiongraded');
                } else if (!$timesubmitted || $status == ASSIGNRECERT_SUBMISSION_STATUS_NEW) {
                    $now = time();
                    if ($due && ($now > $due)) {
                        $overduestr = get_string('overdue', 'assignrecert', format_time($now - $due));
        ...
        if ($row->extensionduedate) {
            ...
            $extensionstr = get_string('userextensiondate', 'assignrecert', $userdate);
    ...
    public function col_userid(stdClass $row) {
        ...
        if (!$row->grade) {
            ...
            $description = get_string('updategrade', 'assignrecert');
        }
        ...
        $submission = $row;
        $flags = $row;
        if ($this->assignmentrecert->get_instance()->teamsubmission) {
        ...
        $submissionsopen = $this->assignmentrecert->submissions_open($row->id,
                                                               true,
                                                               $submission,
                                                               $flags,
                                                               $this->gradinginfo);
        $caneditsubmission = $this->assignmentrecert->can_edit_submission($row->id, $USER->id);

        // Hide for offline assignmentrecerts.
        if ($this->assignmentrecert->is_any_submission_plugin_enabled()) {
            if (!$row->status ||
                    $row->status == ASSIGNRECERT_SUBMISSION_STATUS_DRAFT ||
                    !$this->assignmentrecert->get_instance()->submissiondrafts) {
                ...
                if (!$row->locked) {
                    ...
                    $description = get_string('preventsubmissionsshort', 'assignrecert');
                    ...
                    $urlparams = array('id' => $this->assignmentrecert->get_course_module()->id,
                                       ...
                                       'page'=>$this->currpage);
                    $url = new moodle_url('/mod/assignrecert/view.php', $urlparams);
                    $description = get_string('allowsubmissionsshort', 'assignrecert');
            ...
            if ($submissionsopen &&
                ...
                $urlparams = array('id' => $this->assignmentrecert->get_course_module()->id,
                                   ...
                                   'page'=>$this->currpage);
                $url = new moodle_url('/mod/assignrecert/view.php', $urlparams);
                $description = get_string('editsubmission', 'assignrecert');
        ...
        if (($this->assignmentrecert->get_instance()->duedate ||
                $this->assignmentrecert->get_instance()->cutoffdate) &&
             ...
             $urlparams = array('id' => $this->assignmentrecert->get_course_module()->id,
                                ...
                                'page' => $this->currpage);
             $url = new moodle_url('/mod/assignrecert/view.php', $urlparams);
             $description = get_string('grantextension', 'assignrecert');
        ...
        if ($row->status == ASSIGNRECERT_SUBMISSION_STATUS_SUBMITTED &&
                $this->assignmentrecert->get_instance()->submissiondrafts) {
            $urlparams = array('id' => $this->assignmentrecert->get_course_module()->id,
                               'userid'=>$row->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/assignrecert/view.php', $urlparams);
            $description = get_string('reverttodraftshort', 'assignrecert');
        ...
        if ($row->status == ASSIGNRECERT_SUBMISSION_STATUS_DRAFT &&
                $this->assignmentrecert->get_instance()->submissiondrafts &&
                $caneditsubmission &&
                $submissionsopen &&
                $row->id != $USER->id) {
            $urlparams = array('id' => $this->assignmentrecert->get_course_module()->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/assignrecert/view.php', $urlparams);
            $description = get_string('submitforgrading', 'assignrecert');
        ...
        $ismanual = $this->assignmentrecert->get_instance()->attemptreopenmethod == ASSIGNRECERT_ATTEMPT_REOPEN_METHOD_MANUAL;
        $hassubmission = !empty($row->status);
        $notreopened = $hassubmission && $row->status != ASSIGNRECERT_SUBMISSION_STATUS_REOPENED;
        $isunlimited = $this->assignmentrecert->get_instance()->maxattempts == ASSIGNRECERT_UNLIMITED_ATTEMPTS;
        $hasattempts = $isunlimited || $row->attemptnumber < $this->assignmentrecert->get_instance()->maxattempts - 1;
        ...
        if ($ismanual && $hassubmission && $notreopened && $hasattempts) {
            $urlparams = array('id' => $this->assignmentrecert->get_course_module()->id,
                               ...
                               'page'=>$this->currpage);
            $url = new moodle_url('/mod/assignrecert/view.php', $urlparams);
            $description = get_string('addattempt', 'assignrecert');
     ...
     *
     * @param assignrecert_plugin $plugin Submission plugin or feedback plugin
     ...
     */
    private function format_plugin_summary_with_link(assignrecert_plugin $plugin,
                                                     stdClass $item,
                                                     $returnaction,
                                                     $returnparams) {
        $link = '';
        ...
        $separator = '';
        if ($showviewlink) {
            $viewstr = get_string('view' . substr($plugin->get_subtype(), strlen('assignrecert')), 'assignrecert');
        ...
        if ($plugin->is_visible() && $plugin->is_enabled()) {
            if ($plugin->get_subtype() == 'assignrecertsubmission') {
                if ($this->assignmentrecert->get_instance()->teamsubmission) {
                    ...
                    if ($row->status == ASSIGNRECERT_SUBMISSION_STATUS_REOPENED) {
                        // For a newly reopened submission - we want to show the previous submission in the table.
                        $submission = $this->assignmentrecert->get_user_submission($row->userid, false, $row->attemptnumber - 1);
                    } else {
                        $submission = new stdClass();
                        $submission->id = $row->submissionid;
                        $submission->timecreated = $row->firstsubmission;
                        $submission->timemodified = $row->timesubmitted;
                        $submission->assignmentrecert = $this->assignmentrecert->get_instance()->id;
                ...
                if ($row->gradeid) {
                    ...
                    $grade->assignmentrecert = $this->assignmentrecert->get_instance()->id;
     ...
     *
     * @return string the assignmentrecert name
     */
    public function get_assignmentrecert_name() {
        return $this->assignmentrecert->get_instance()->name;
    }
    ...
    public function get_course_module_id() {
        return $this->assignmentrecert->get_course_module()->id;
    }
    ...
    public function get_course_id() {
        return $this->assignmentrecert->get_course()->id;
    }
    ...
    public function get_course_context() {
        return $this->assignmentrecert->get_course_context();
    }

    /**
     * Return things to the renderer.
     *
     * @return bool Does this assignmentrecert accept submissions
     */
    public function submissions_enabled() {
        return $this->assignmentrecert->is_any_submission_plugin_enabled();
    }
    ...
    public function can_view_all_grades() {
        $context = $this->assignmentrecert->get_course_context();
    ...
    public function get_sort_columns() {
        ...
        $assignmentrecert = $this->assignmentrecert->get_instance();
        if (empty($assignmentrecert->blindmarking)) {
