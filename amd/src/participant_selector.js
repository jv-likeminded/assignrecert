...
/**
 * Custom auto-complete adapter to load users from the assignmentrecert list_participants webservice.
 *
 * @module     mod_assignrecert/participants_selector
 * @copyright  2015 Damyon Wiese <damyon@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
define(["core/ajax", "jquery", "core/templates"], function (
  ajax,
  $,
  templates
) {
  return /** @alias module:mod_assignrecert/participants_selector */ {
    ...
    transport: function (selector, query, success, failure) {
      var assignmentrecertid = $(selector).attr("data-assignmentrecertid");
      var groupid = $(selector).attr("data-groupid");
      var filters = $(
        '[data-region="configure-filters"] input[type="checkbox"]'
      );
      var filterstrings = [];
      ...
      ajax
        ...
        .then(function (results) {
          var promises = [];
          var identityfields = $("[data-showuseridentity]")
          ...
          $.each(results, function (index, user) {
            ...
            if (
              filterstrings.filter_grantedextension &&
              !user.grantedextension
            ) {
              show = false;
            }
            if (show) {
              ...
              promises.push(
                templates
                  .render("mod_assignrecert/list_participant_user_summary", ctx)
                  .then(function (html) {
