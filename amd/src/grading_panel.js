 ...
 *
 * @module     mod_assignrecert/grading_panel
...
define([
  ...
  "mod_assignrecert/grading_events",
], function (
  $,
  Y,
  notification,
  ...
  GradingPanel.prototype._submitForm = function (event, nextUserId) {
    ...
    var assignmentrecertid = this._region.attr("data-assignmentrecertid");

    // Now we can continue...
    ajax.call([
      {
        methodname: "mod_assignrecert_submit_grading_form",
        args: {
   ...
   */
  GradingPanel.prototype._handleFormSubmissionResponse = function (
    formdata,
    nextUserId,
    response
  ) {
    ...
    if (response.length) {
      ...
      str
        .get_strings([
          ...
          { key: "gradechangessaveddetail", component: "mod_assignrecert" },
        ])
        .done(function (strs) {
  ...
  GradingPanel.prototype._chooseAttempt = function (e) {
    ...
    str
      .get_strings([
        { key: "viewadifferentattempt", component: "mod_assignrecert" },
  ...
  GradingPanel.prototype._addPopoutButtons = function (selector) {
    ...
    templates
      .render("mod_assignrecert/popout_button", {})
      .done(
        function (html) {
          var parents = region
            .find(
   ...
   */
  GradingPanel.prototype._refreshGradingPanel = function (
    event,
    userid,
    submissiondata,
    attemptnumber
    ...
    window.M.util.js_pending("mod-assignrecert-loading-user");
    // First insert the loading template.
    templates
      .render("mod_assignrecert/loading", {})
      .done(
        function (html, js) {
          // Update the page.
          this._niceReplaceNodeContents(this._region, html, js).done(
            function () {
              if (userid > 0) {
                this._region.show();
                      ...
                      this._niceReplaceNodeContents(this._region, html, js)
                        .done(
                          function () {
                            ...
                            $('[data-region="attempt-chooser"]').on(
                              "click",
                ...
                $(document).trigger("finish-loading-user");
                // Tell behat we are friends again.
                window.M.util.js_complete("mod-assignrecert-loading-user");
              }
            }.bind(this)
          );
        }.bind(this)
  ...
  GradingPanel.prototype.registerEventListeners = function () {
    ...
    docElement.on(
      GradingEvents.COLLAPSE_GRADE_PANEL,
      function () {
        this.collapsePanel();
      }.bind(this)
    );
    ...
    docElement.on(
      GradingEvents.COLLAPSE_REVIEW_PANEL,
      function () {
        this.expandPanel();
      }.bind(this)
    );
    ...
    docElement.on(
      GradingEvents.EXPAND_GRADE_PANEL,
      function () {
        this.expandPanel();
      }.bind(this)
    );
