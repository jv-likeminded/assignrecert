 ...
 *
 * @module     mod_assignrecert/grading_navigation
...
define([
  ...
  "core/ajax",
  "mod_assignrecert/grading_form_change_checker",
  ...
  var GradingNavigation = function (selector) {
    ...
    var configPanel = $(
      document.getElementById(toggleLink.attr("aria-controls"))
    );
    ...
    var userid = $('[data-region="grading-navigation-panel"]').data(
      "first-userid"
    );
    if (userid) {
    ...
    str
      .get_string("changeuser", "mod_assignrecert")
      .done(function (s) {
        autocomplete.enhance(
          "[data-action=change-user]",
          false,
          "mod_assignrecert/participant_selector",
          s
        );
      })
  ...
  /**
   * Load the list of all users for this assignmentrecert.
   *
   * @private
  ...
  GradingNavigation.prototype._loadAllUsers = function () {
    ...
    var assignmentrecertid = select.attr("data-assignmentrecertid");
    var groupid = select.attr("data-groupid");

    ajax.call([
      {
        methodname: "mod_assignrecert_list_participants",
        args: {
          assignrecertid: assignmentrecertid,
          groupid: groupid,
          filter: "",
          onlyids: true,
        },
   ...
   * @param {Event} event
   */
  GradingNavigation.prototype._checkClickOutsideConfigureFilters = function (
    event
  ) {
    var configPanel = this._region.find('[data-region="configure-filters"]');

    if (
      !configPanel.is(event.target) &&
      configPanel.has(event.target).length === 0
    ) {
      var toggleLink = this._region.find('[data-region="user-filters"]');
      ...
      $(document).off("click.mod_assignrecert_grading_navigation");
    }
  };
  ...
  GradingNavigation.prototype._filterChanged = function (event) {
    ...
    if (filterlist.length) {
      ...
      this._region
        .find('[data-region="user-filters"] span.accesshide')
        .text(filterlist.join(", "));
    } else {
      str
        .get_string("nofilters", "mod_assignrecert")
        .done(
          function (s) {
            // Totara: Ensure text is only added to the accessibility element.
            this._region
  ...
  GradingNavigation.prototype._selectNoUser = function () {
    ...
    if (checker.checkFormForChanges('[data-region="grade-panel"] .gradeform')) {
      ...
      str
        .get_strings([
          { key: "unsavedchanges", component: "mod_assignrecert" },
          { key: "unsavedchangesquestion", component: "mod_assignrecert" },
          { key: "saveandcontinue", component: "mod_assignrecert" },
  ...
  GradingNavigation.prototype._toggleExpandFilters = function (event) {
    ...
    if (expanded) {
      ...
      $(document).on(
        "click.mod_assignrecert_grading_navigation",
  ...
  GradingNavigation.prototype._refreshCount = function () {
    ...
    if (isNaN(userid) || userid <= 0) {
      ...
      str
        .get_string("xofy", "mod_assignrecert", param)
        .done(
          function (s) {
