...
/**
 * This file contains the functions for assignrecert_plugin abstract class
 *
 *
...
/**
 * Abstract class for assignrecert_plugin (submission/feedback).
 *
 * @package   mod_assignrecert
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class assignrecert_plugin {

    /** @var assignrecert $assignmentrecert the assignmentrecert record that contains the global
     *              settings for this assignrecert instance
     */
    protected $assignmentrecert;
    /** @var string $type assignmentrecert plugin type */
    private $type = '';
     ...
     *
     * @param assignrecert $assignmentrecert
     * @param string $type
     */
    public final function __construct(assignrecert $assignmentrecert, $type) {
     ...
     * form for display in the settings page for the assignmentrecert.
    ...
    /**
     * The assignmentrecert subtype is responsible for saving it's own settings as the database table for the
     ...
     *
     * @return string - either 'assignrecertsubmission' or 'feedback'
     */
     ...
     *
     * @param stdClass $submissionorgrade - assignrecert_submission or assignrecert_grade.
    ...
    /**
     * Get any additional fields for the submission/grading form for this assignmentrecert.
     *
    ...
    public final function has_admin_settings() {
        ...
        $pluginroot = $CFG->dirroot . '/mod/assignrecert/' . substr($this->get_subtype(), strlen('assignrecert')) . '/' . $this->get_type();
    ...
    public final function set_config($name, $value) {
        ...
        $dbparams = array('assignmentrecert'=>$this->assignmentrecert->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                          'plugin'=>$this->get_type(),
                          'name'=>$name);
        $current = $DB->get_record('assignrecert_plugin_config', $dbparams, '*', IGNORE_MISSING);

        if ($current) {
            $current->value = $value;
            return $DB->update_record('assignrecert_plugin_config', $current);
        } else {
            $setting = new stdClass();
            $setting->assignmentrecert = $this->assignmentrecert->get_instance()->id;
            $setting->subtype = $this->get_subtype();
            $setting->plugin = $this->get_type();
            $setting->name = $name;
            $setting->value = $value;

            return $DB->insert_record('assignrecert_plugin_config', $setting) > 0;
        }
    }
    ...
    public final function get_config($setting = null) {
        ...
        if ($setting) {
            ...
            if ($assignmentrecert) {
                $dbparams = array('assignmentrecert'=>$assignmentrecert->id,
                                  'subtype'=>$this->get_subtype(),
                                  'plugin'=>$this->get_type(),
                                  'name'=>$setting);
                $result = $DB->get_record('assignrecert_plugin_config', $dbparams, '*', IGNORE_MISSING);
                if ($result) {
                    return $result->value;
                }
            }
        ...
        $dbparams = array('assignmentrecert'=>$this->assignmentrecert->get_instance()->id,
                          'subtype'=>$this->get_subtype(),
                           'plugin'=>$this->get_type());
        $results = $DB->get_records('assignrecert_plugin_config', $dbparams);
    ...
    /**
     * Return true if this plugin can upgrade an old Moodle 2.2 assignmentrecert of this type
     * and version.
     *
     * @deprecated since Totara 13
     * @param string $type The old assignmentrecert subtype
     * @param int $version The old assignmentrecert version
    ...
    /**
     * Upgrade the settings from the old assignmentrecert to the new one
     *
     * @deprecated since Totara 13
     * @param context $oldcontext The context for the old assignmentrecert module
     * @param stdClass $oldassignmentrecert The data record for the old assignmentrecert
     * @param string $log Record upgrade messages in the log
     * @return bool true or false - false will trigger a rollback
     */
    public function upgrade_settings(context $oldcontext, stdClass $oldassignmentrecert, & $log) {
        debugging(__METHOD__ . ' has been deprecated and should no longer be used', DEBUG_DEVELOPER);

        $params = array('type'=>$this->type, 'subtype'=>$this->get_subtype());
        $log .= ' ' . get_string('upgradenotimplemented', 'mod_assignrecert', $params);
        return false;
    }

    /**
     * Upgrade the submission from the old assignmentrecert to the new one
     *
     * @deprecated since Totara 13
     ...
     * @param stdClass $submissionorgrade assignrecert_submission or assignrecert_grade The new submission or grade
     * @param string $log Record upgrade messages in the log
     * @return boolean true or false - false will trigger a rollback
     */
    public function upgrade(context $oldcontext,
                            stdClass $oldassignmentrecert,
        ...
        $params = array('type'=>$this->type, 'subtype'=>$this->get_subtype());
        $log = $log . ' ' . get_string('upgradenotimplemented', 'mod_assignrecert', $params);
        return false;
    }
    ...
    /**
     * The assignmentrecert has been deleted - remove the plugin specific data
     *
     * @return bool
     */
    ...
    /**
     * Is this assignmentrecert plugin empty? (ie no submission or feedback)
     * @param stdClass $submissionorgrade assignrecert_submission or assignrecert_grade
     * @return bool
     */
    ...
    public function get_file_info($browser, $filearea, $itemid, $filepath, $filename) {
        ...
        if ($this->get_subtype() == 'assignrecertsubmission') {
            if ($itemid) {
                $record = $DB->get_record('assignrecert_submission', array('id'=>$itemid), 'userid', IGNORE_MISSING);
                if (!$record) {
                    return null;
                }
                if (!$this->assignmentrecert->can_view_submission($record->userid)) {
                    return null;
                }
            }
        } else {
        ...
        if (!($storedfile = $fs->get_file($this->assignmentrecert->get_context()->id,
    ...
    /**
     * This allows a plugin to render a page in the context of the assignmentrecert
     *
     * If the plugin creates a link to the assignmentrecert view.php page with
     * The following required parameters:
     *      id=coursemoduleid
     *      plugin=type
     *      pluginsubtype=assignrecertfeedback|assignrecertsubmission
     ...
     * right below the activity's "intro" section on the main assignmentrecert page.
     *
     * @return string
     */
     ...
     * enabled/disabled per assignmentrecert instance.
     *
     * @return bool
     */
