 ...
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_assignrecert_search_testcase extends advanced_testcase {

    /**
     * Test for assignrecert file attachments.
     *
     * @return void
     */
    public function test_attach_files() {
        ...
        $assignareaid = \core_search\manager::generate_areaid('mod_assignrecert', 'activity');
        ...
        $assignrecert = $this->getDataGenerator()->create_module('assignrecert', $record);
        ...
        $filerecord = array(
            ...
            'filearea'  => ASSIGNRECERT_INTROATTACHMENT_FILEAREA,
            'itemid'    => 0,
            'filepath'  => '/'
        );
        ...
        for ($i = 1; $i <= 4; $i++) {
            ...
            $fs->create_file_from_string($filerecord, 'Test assignrecert file '.$i);
        }

        // And a fifth in a sub-folder.
        $filerecord['filename'] = 'myfile5';
        $filerecord['filepath'] = '/subfolder/';
        $fs->create_file_from_string($filerecord, 'Test assignrecert file 5');

        // Returns the instance as long as the area is supported.
        $searcharea = \core_search\manager::get_search_area($assignareaid);
        $this->assertInstanceOf('\mod_assignrecert\search\activity', $searcharea);
