...
/**
 * Unit tests for (some of) mod/assignrecert/markerallocaion_test.php.
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_assignrecert_markerallocation_testcase extends advanced_testcase {
    ...
    public function test_markerusers() {
        ...
        $assignelement = $this->getDataGenerator()->create_module('assignrecert', $record, $modulesettings);
        ...
        if (!empty($managerrole)) {
            ...
            $idassignment = $this->getDataGenerator()->role_assignrecert($managerrole->id, $user4->id);
        }
        ...
        $oldmarkers = get_users_by_capability(context_course::instance($this->course->id), 'mod/assignrecert:grade', '', $sort);
        // New code, it must return 2 users: teacher1 and teacher2.
        $newmarkers = get_enrolled_users(context_course::instance($this->course->id), 'mod/assignrecert:grade', 0, 'u.*', $sort);
