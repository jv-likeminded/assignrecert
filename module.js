...
M.mod_assignrecert.init_tree = function (Y, expand_all, htmlid) {
...
M.mod_assignrecert.init_grading_table = function (Y) {
    ...
    if (batchform) {
      batchform.on("submit", function (e) {
        ...
        if (selectedusers.length == 0) {
          alert(M.util.get_string("nousersselected", "assignrecert"));
          e.preventDefault();
        } else {
          ...
          if (action.indexOf(prefix) == 0) {
            ...
            confirmmessage = M.util.get_string(
              "batchoperationconfirm" + action,
              "assignrecertfeedback_" + plugin
            );
          } else {
            confirmmessage = M.util.get_string(
              "batchoperationconfirm" + operation.get("value"),
              "assignrecert"
            );
          }
...
M.mod_assignrecert.init_grading_options = function (Y) {
  Y.use("node", function (Y) {
...
M.mod_assignrecert.init_plugin_summary = function (
  Y,
  subtype,
  type,
  submissionid
