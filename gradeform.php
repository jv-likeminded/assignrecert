...
/**
 * Assignment Recert grade form
 *
 * @package   mod_assignrecert
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_assignrecert_grade_form extends moodleform {
    /** @var assignmentrecert $assignmentrecert */
    ...
    public function definition() {
        ...
        list($assignmentrecert, $data, $params) = $this->_customdata;
        // Visible elements.
        $this->assignmentrecert = $assignmentrecert;
        $assignmentrecert->add_grade_form_elements($mform, $data, $params);
    ...
    public function validation($data, $files) {
        ...
        if ($instance->grade > 0) {
            if (unformat_float($data['grade'], true) === false && (!empty($data['grade']))) {
                $errors['grade'] = get_string('invalidfloatforgrade', 'assignrecert', $data['grade']);
            } else if (unformat_float($data['grade']) > $instance->grade) {
                $errors['grade'] = get_string('gradeabovemaximum', 'assignrecert', $instance->grade);
            } else if (unformat_float($data['grade']) < 0) {
                $errors['grade'] = get_string('gradebelowzero', 'assignrecert');
            }
        } else {
            // This is a scale.
            if ($scale = $DB->get_record('scale', array('id'=>-($instance->grade)))) {
                ...
                if ((int)$data['grade'] !== -1 && !array_key_exists((int)$data['grade'], $scaleoptions)) {
                    $errors['grade'] = get_string('invalidgradeforscale', 'assignrecert');
                }
            }
        }
        return $errors;
