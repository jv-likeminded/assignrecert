...
/**
 * This page handles editing and creation of assignrecert overrides
 *
...
$pagetitle = get_string('editoverride', 'assignrecert');

$override = null;
if ($overrideid) {

    if (! $override = $DB->get_record('assignrecert_overrides', array('id' => $overrideid))) {
        print_error('invalidoverrideid', 'assignrecert');
    }

    list($course, $cm) = get_course_and_cm_from_instance($override->assignrecertid, 'assignrecert');
...
$url = new moodle_url('/mod/assignrecert/overrideedit.php');
if ($action) {
...
// Merge assignrecert defaults with data.
$keys = array('duedate', 'cutoffdate', 'allowsubmissionsfromdate');
foreach ($keys as $key) {
    if (!isset($data->{$key}) || $reset) {
        $data->{$key} = $assignrecertinstance->{$key};
    }
}
...
if ($action === 'duplicate') {
    ...
    $pagetitle = get_string('duplicateoverride', 'assignrecert');
}

$overridelisturl = new moodle_url('/mod/assignrecert/overrides.php', array('cmid' => $cm->id));
if (!$groupmode) {
    $overridelisturl->param('mode', 'user');
}

// Setup the form.
$mform = new assignrecert_override_form($url, $cm, $assignrecert, $context, $groupmode, $override);
$mform->set_data($data);
...
if ($mform->is_cancelled()) {
    ...
    // Process the data.
    $fromform->assignrecertid = $assignrecertinstance->id;

    // Replace unchanged values with null.
    foreach ($keys as $key) {
        if (($fromform->{$key} == $assignrecertinstance->{$key})) {
            $fromform->{$key} = null;
        }
    }
    ...
    if ($userorgroupchanged) {
        ...
        if ($oldoverride = $DB->get_record('assignrecert_overrides', $conditions)) {
            ...
            $assignrecert->delete_override($oldoverride->id);
        }
    }
    ...
    if (!empty($override->id)) {
        ...
        $DB->update_record('assignrecert_overrides', $fromform);
        ...
        unset($fromform->id);
        $fromform->id = $DB->insert_record('assignrecert_overrides', $fromform);
        if ($groupmode) {
            $fromform->sortorder = 1;

            $overridecountgroup = $DB->count_records('assignrecert_overrides',
                array('userid' => null, 'assignrecertid' => $assignrecertinstance->id));
            $overridecountall = $DB->count_records('assignrecert_overrides', array('assignrecertid' => $assignrecertinstance->id));
            ...
            $DB->update_record('assignrecert_overrides', $fromform);
            reorder_group_overrides($assignrecertinstance->id);
        }
    ...
    assignrecert_update_events($assignrecert, $fromform);
...
echo $OUTPUT->header();
echo $OUTPUT->heading(format_string($assignrecertinstance->name, true, array('context' => $context)));
